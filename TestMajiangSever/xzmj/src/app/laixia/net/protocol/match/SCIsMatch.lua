...
local function onPacketSCIsMatch(packet)
    if packet.data.GameID == 4 or packet.data.GameID == 1 then
        local function _onReadyCall()
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_VERSIONOPEN_WINDOW)           --请求功能开关
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_HEARTTICK_WINDOW)          --发送心跳

            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_RELIEFINFO_WINDOW)       --请求救济金次数
            --请求活动接口
            laixia.LocalPlayercfg.LaixiaisActivity = true
            ...
            if packet.data.isMatch == 0 then
                ...
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"重新连接成功，比赛继续。")
            elseif  packet.data.isMatch == 1 then
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_ISONLINEROOM_WINDOW)
            end
        end
        local nnet = laixia.net.Net;
...
return
    {
        ID = _LAIXIA_PACKET_SC_IsMatchID,
        name = "SCIsMatch",
        data_array =
        {

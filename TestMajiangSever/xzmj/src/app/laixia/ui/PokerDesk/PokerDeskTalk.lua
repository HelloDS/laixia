...
function PokerDeskTalk:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_TALKING_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_TALKING_WINDOW, handler(self, self.destroy))
...
function PokerDeskTalk:sendsever(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if str == "" then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"亲，还是说点啥吧！")
            return
        end
        --普通牌桌发送对话，新手牌桌不发送对话
        ...
        local mesg = { }
        mesg.seatId = laixia.LocalPlayercfg.LaixiaMySeat

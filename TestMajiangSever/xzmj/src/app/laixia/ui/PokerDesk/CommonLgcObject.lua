            ...
            for k1 = 1, #selectPokers do
                for k2 = 1, length do
                        ...
                        if laixia.LocalPlayercfg.LaixiaOutCardsIndex ~= nil  then
                            if laixia.LocalPlayercfg.LaixiaOutCardsIndex[1].CardValue < 52 then  -- 用于处理王的问题--出的牌有可能是王，因此在做处理的时候要做这个处理
                                outpoker =(laixia.LocalPlayercfg.LaixiaOutCardsIndex[1].CardValue) % 13
                            else
                                outpoker =(laixia.LocalPlayercfg.LaixiaOutCardsIndex[1].CardValue) % 13 + 13
                            end
                        end
                    ...
                    if laizipoker >= 0 and laizipoker  ==(pokerNum) then        -- 癞子牌有可能为0 --出来选择的牌为癞子牌
                        if laizi_num == length then                            -- 当癞子的个数等于长度--癞子作为本身时其值要大于所出的值
                            if length ~= 4 then                    -- 癞子作为本身的时候
                                if laixia.LocalPlayercfg.LaixiaOutCardsIndex ~= nil and pokerNum > outpoker then
                                    flag = true
                                    for i = 1, length do

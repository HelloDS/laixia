...
local laixia = laixia;
laixia.ui.CBaseDialog = CBaseDialog;
...
function CBaseDialog:isActiveDialog()
    local winManager = laixia.ui.WindowManager
    ...
    if(not self.mIsLoad) then
        if self.hDialogType ~= DialogTypeDef.DEFINE_INNER_DIALOG and self.hDialogType ~= DialogTypeDef.DEFINE_SINGLE_DIALOG then
            ...
            local function touchEnded(touch, event)
                if  laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow=="WonderfulWindow" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "DailyLoginWindow" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "TPFirstCharge" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "PersonalCenterWindow" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "GameSets" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "UserInfoWindow" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "StatinMessage" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "MyBagWindow" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "GameAbout" or laixia.LocalPlayercfg.LaixiaCurrentNomarlWindow == "TPNoticeWindow" then
                    self:destroy()
                end    
            end
            ...
            if(string.find(self.mDialogItemData.Resource,".csb")) then
                self.mInterfaceRes = laixia.Layout.loadLayer(self.mDialogItemData.Resource)
            else
...
function CBaseDialog:show(...)
    ...
    if(self.hDialogType == DialogTypeDef.DEFINE_SINGLE_DIALOG) then
        laixia.ui.WindowManager:onTop(self)
    elseif(self.hDialogType == DialogTypeDef.DEFINE_INNER_DIALOG) then
        laixia.ui.WindowManager:intoInnerList(self)
    elseif(self.hDialogType == DialogTypeDef.DEFINE_NORMAL_DIALOG) then
        laixia.ui.WindowManager:intoNormalList(self)
    end

    laixia.ui.WindowManager:updateZOrder()
    self:onShow(...)

    laixia.ui.WindowManager:setActiveWindow(self);
...
function CBaseDialog:destroy()
    ...
    if(not self.mIsLoad ) then
        if self.hDialogType == DialogTypeDef.DEFINE_NORMAL_DIALOG then
            laixia.ui.WindowManager:destoryNormalWindowsByList(self)
        end
        return;
    end
    ...
    laixia.ui.WindowManager:destoryInnerWindowsByKey(self:getName())
    if(self.hDialogType == DialogTypeDef.DEFINE_SINGLE_DIALOG) then
        if(laixia.ui.WindowManager.mTopWindow == self) then
            laixia.ui.WindowManager.mTopWindow = nil
            laixia.ui.WindowManager:destoryAllNormalWindows()
        end
    elseif self.hDialogType == DialogTypeDef.DEFINE_NORMAL_DIALOG then
    ...
    if(self:isActiveDialog()) then
        laixia.ui.WindowManager:updateActiveWindow();
    end

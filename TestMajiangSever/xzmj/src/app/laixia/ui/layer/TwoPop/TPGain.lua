...
function Gain:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_GAIN_WINDOW, handler(self, self.show))
end
...
function Gain:onShow(msg)
     ...
     self.mIsShow = false
     self.mItemArray = {}
     -- local nodeBottom = laixia.ani.CObjectAnimationManager:playAnimationAt(self.mInterfaceRes,"Gain","Default Timeline",
     -- function()  
     -- end )  
...
function Gain:addItemMsg(cell,data,copyNum)
   ...
   if data.noshowNum then  --如果为空也显示
    ...
    self:GetWidgetByName("Gain_Libao_Num"..str, cell):setLocalZOrder(50)

...
function TPVirtualExchange:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_REDEEMVIRTUAL_WINDOW, handler(self, self.show))
end
...
function TPVirtualExchange:SendExchangePacket(sender, eventtype)
    if eventtype == ccui.TouchEventType.ended then
        ...
        if str == "" then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "需要输入号码！")
            return
        end

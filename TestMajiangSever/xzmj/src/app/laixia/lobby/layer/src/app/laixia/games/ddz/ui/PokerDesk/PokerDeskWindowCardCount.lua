...
function PokerDeskWindowCardCount:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_BUYRECORDER_WINDOW,handler(self,self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_BUYRECORDER_WINDOW,handler(self,self.destroy))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CHANGE_BUYRECORDER_WINDOW,handler(self,self.change))
end
...
function PokerDeskWindowCardCount:onShow(aiData)
    ...
    if laixiaddz.LocalPlayercfg.mAlreadyBuyJipaiqiUserNumber then
        local number = math.random(10,50)
        laixiaddz.LocalPlayercfg.mAlreadyBuyJipaiqiUserNumber = laixiaddz.LocalPlayercfg.mAlreadyBuyJipaiqiUserNumber+number
    else
        laixiaddz.LocalPlayercfg.mAlreadyBuyJipaiqiUserNumber = math.random(5000,8000)
    end
    local str = "已有 " .. laixiaddz.LocalPlayercfg.mAlreadyBuyJipaiqiUserNumber .. "人 购买记牌器"

...
-----------------------------------------------------------------------------------
        ...
        self.matchInsId = msg.data.match_ins_id
...
function CardTableDialog:OnInitSynchronousFunction(msg)
    ...
    dump(msg.data.Players,"msg.data.Players")
    ...
    --叫牌阶段
    dump(msg.data.Stage,"msg.data.Players")
...
function CardTableDialog:synchronousStepDataFunction(msg)
    ...
    dump(self.mClockTime,"self.mClockTime")
...
function CardTableDialog:cancelHostingFunction(msg)
    ...
    dump("cancelHostingFunction",self.mMySeatID)
    dump("cancelHostingFunction111",msg.data.Seat)
...
function CardTableDialog:createResultDataFunction(data)
    ...
    dump(self.mBottomPID,"self.mBottomPID")
    for i = 1,3 do
   ...
   if self.mSelfRoomID ~= 50 then
    ...
    dump(_laixiaddz_EVENT_SHOW_TABLERESULT_WINDOW,"_laixiaddz_EVENT_SHOW_TABLERESULT_WINDOW")
...
function CardTableDialog:closeFunction()
    ...
    if self.mPokerDeskType and self.mPokerDeskType == 2 then
        ...
        if self.isPlay == true then
            ...
            scene:popUpTips("牌局结束时才能退出")
            -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"牌局结束时才能退出")
        else
            local stream =  laixia.Packet.new("gameTableExit", "LXG_GAME_EXIT")
            ...
            laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event)
                if event.dm_error == 0 then
                    ...
                    local scene = cc.Director:getInstance():getRunningScene()
                    scene:popUpTips("游戏中退出牌桌失败！"..event.dm_error)
                end 
            end) 
        end
        return
                ...
                OnLeftCallFun = function()
                    ...
                    laixia.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
                        ...
                        if event.dm_error == 0 then
                            ...
                            -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_GAMELIST_GOGAMELIST)
                            local scene = cc.Director:getInstance():getRunningScene()
                            scene:popUpTips("退出比赛失败！",event.error_msg)
                        end 
                    end)                                                                            
                end,
...
function CardTableDialog:onCallPoints(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        local packet = l_PacketObj.new("CGCallLandlord", "call_point")
        packet:setValue("room_id", self.mRoomID)
        packet:setValue("table_id", self.hDeskID)
        packet:setValue("type", 0)
        packet:setValue("chip", point)
        laixiaddz.net.sendPacket(packet)
        self:ChangeTouchbarFunction(self.mCallPointBar, false)
    end
end
...
function CardTableDialog:onPlayerInfoFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        laixia.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
            ...
            if event.dm_error == 0 then
                local player = {}
                player.Name = data1.nick or ""
                player.imgPath = data1.portrait or ""
                player.ID = data1.uid or 0
                player.gold = data1.gold or 0 
                local stream =  laixia.Packet.new("person", "LXG_COUNT_INFO")
                stream:setReqType("get")
                stream:setValue("uids", PID)
                laixia.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
                    local data1 = event
                    if data1.dm_error == 0 then
                        if #data1.data == 0 then
                            player.winParam = 0
                            player.winNum = 0
                        end
                        local data = data1.data[1]
                        if data then
                            player.winParam = data.probability or 0
                            player.winNum = data.runnerup or 0
                        end
                    else
                        player.winParam = 0
                        player.winNum = 0
...
function CardTableDialog:OnbeginFunction(msg)
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
        local roomID = laixiaddz.LocalPlayercfg:room_query_Id()
        local mRoomInfo = l_RoomDataMgr:query("roomid",laixiaddz.LocalPlayercfg:room_query_Id());
        if mRoomInfo then
...
function CardTableDialog:sendContinueGame()
    ...
    stream:setValue("score",mTicketGold)
    stream:setPostData("uid", laixiaddz.LocalPlayercfg.LaixiaPlayerID)
    local function cb(data)
        dump(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW")
        dump(data.dm_error,"dm_error")

    --TODo 游戏场暂时写死
        dump("roomID",self.mRoomID)
        local data = {}
        data.RoomID = self.mRoomID
        data.RoomType = 2
...
function CardTableDialog:OnSendContinueGameFunction()
    if self.nomoney then
        ...
        if self.isDownState == 0 then
            ...
            if roominfo and roominfo1 and laixiaddz.LocalPlayercfg.MatchGold<roominfo["minGold"] and laixiaddz.LocalPlayercfg.MatchGold>roominfo1["minGold"] then
                self.isDownState = 1
                --self:GameSendLeaveRoomFunction()

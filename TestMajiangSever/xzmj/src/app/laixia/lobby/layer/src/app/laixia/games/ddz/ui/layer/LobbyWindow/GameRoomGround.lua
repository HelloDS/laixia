...
function GameRoomGround:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SEND_ROOM_WINDOW, handler(self, self.sendRoomListPacket))  
...
function GameRoomGround:sendRoomListPacket(msg)
    local data = msg.data
    self.mRoomType = data
    local stream = laixiaddz.Packet.new("EnterListRoom", _LAIXIA_PACKET_CS_ListRoomID)
...
--    if laixiaddz.LocalPlayercfg.LaixiaGoldCoin >= maxCoin and maxCoin >0 then 
--        local callback = function()

--            if self.mRoomType ~= nil then
--                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SEND_QUICKGAME_WINDOW,self.mRoomType)
--            else
--                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SEND_QUICKGAME_WINDOW,0)
--            end

--        end
--        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, {Text="您太土豪了，还是去更高级的房间玩吧~ " , OnCallFunc=callback})
--        return false 
--    end
    return mTicketGold
end 

...
local function onPacketSCAppleDismiss(packet)
    ...
    if ui.AppleDismissLayer.mIsShow == false then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW,{data = data})
    elseif ui.AppleDismissLayer.mIsShow == true then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_UPDATE_APPLYDISMISS_WINDOW,{data = data})
    end
    --     end
end
...
return
    {
        ID = _laixiaddz_PACKET_SC_AppleDismissID,
        name = "SCAppleDismiss",
        data_array =
        {
            --{"StatuID", Type.Short},

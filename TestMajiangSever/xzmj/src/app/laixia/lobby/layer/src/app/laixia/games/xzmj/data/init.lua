...
local stringSplit = laixiaTools.stringSplit;
local stringSplitColor = laixiaTools.stringSplitColor;
local colorSplitGroups = laixiaTools.stringSplitColors;
...
local itemsLoad = function(path)
    ...
    for i,v in ipairs(buf) do
        ret_buf[i] = laixia.UItools.stringSplit(v,"|",{"PresentItemID2","Length"},nil)
    end
    return ret_buf;
end
...
function db2:onInit()
    ...
    self:load{
        {"window","data/uiConfigure.json"},
    }

    self.resLoader_t = {
        {"items","data/xzmj/items.txt",itemsLoad},

...
local function SCBalancePacket(packet)
    --dump(packet.data)
    laixiaddz.logGame("牌桌结算消息 _laixiaddz_PACKET_SC_BalanceID") 
    dumpGameData(packet.data,"")

    if  laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "CardTableDialog" and ui.CardTableDialog.mRoomID == packet.data.RoomID then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SPRING_LANDLORDTABLE_WINDOW,packet.data)
    else
...
SCBalance =   -- 结算,
{
    ID = _laixiaddz_PACKET_SC_BalanceID,

...
local function onPacketSCCreateDelRet(packet)
    if StatusCode.new(packet.data.Status):isOK() == true   then
        ...
        local str = "房间已经解散"
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,str) --飘字提示
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_APPLYDISMISS_WINDOW)
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_HALL_WINDOW);   --返回大厅
    end
end

return
    {
        ID = _LAIXIA_PACKET_SC_CreateDelRetID,
        name = "SCCreateDelRet",
        data_array =
        {
            {"Status", Type.Short},

...
local schedul = require("framework.scheduler")

function DownloaderHead:ctor(...)           
    self:reset()
    schedul.scheduleUpdateGlobal(handler(self, self.tick),1/20)
end 
...
--想下载队列中加入一个下载人物
function DownloaderHead:pushTask(playerID, url,mInGame,obj)
...
function DownloaderHead:onResponsePost(event)
    ...
    elseif(task.mInGame==2) then --大厅
        ...
        self.parent_obj:onHeadDoSuccess(event)

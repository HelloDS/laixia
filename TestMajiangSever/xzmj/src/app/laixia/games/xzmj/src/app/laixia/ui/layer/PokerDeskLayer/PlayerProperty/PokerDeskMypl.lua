...
function PokerDeskMypl:ZhuaPai( _Num )
    ...
    --- 抓牌抓到碰过的牌 提示杠
    local tb = {}
    for k,v in pairs(self.mPengPTb) do
        if _Num == v then
            table.insert( tb, pkcf.MJONGTY.MINGGANG )
            self.mLayer:ShowPGHGui( tb )
            break
        end
    end
...
function PokerDeskMypl:Inspect(  )

    local _val = pkModel.mPlChupaiVal

    local tb =  pkModel:InspectMj( _val, self.mMySPValTb)
    if tb == nil then
        return
    end
    local len = #tb

...
local function TableSynchronousPacket(packet)
    ...
    laixiaddz.LocalPlayercfg.laixiaddzLandlordSeat = packet.data.BottomSeat
    for k, v in ipairs(packet.data.Players) do
        if v.PID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
            laixiaddz.LocalPlayercfg.laixiaddzMySeat = v.Seat
            laixiaddz.LocalPlayercfg.MatchGold = v.Coin
            break
        end
    end
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_INITSYN_LANDLORDTABLE_WINDOW, packet.data)

end

SCTableSync =
    {
        ID = _laixiaddz_PACKET_SC_TableSyncID,
        -- 牌桌同步,
        name = "SCTableSync",
        data_array =
        {

...
function Parser:_generatePacket()
    ...
    if(Protocol.getProtocol(id) == nil)then
        --
        laixiaddz.logError("Error Generate Packet ID :"..id)
        --ObjectEventDispatch:pushEvent(EVENT_RE_CONNECT_REQ)
        if id == 40 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, { Text = "账号登陆异常请重启", OnCallFunc = function() app:exit()() end })
        elseif msgLen<0 then
        --ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, { Text = "网络异常，点击尝试重新连接", OnCallFunc = function() laixiaddz.net.readyForReConnect(); end })
        else
        --			--兼容德州的消息

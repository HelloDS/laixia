...
function GameTypeWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_GAMETYPE_WINDOW, handler(self, self.show))
end
...
function GameTypeWindow:onShowRoomGround(sender, eventtype)
    if eventtype == ccui.TouchEventType.ended then
        ...
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SEND_ROOM_WINDOW,game_type)
    end
end
function GameTypeWindow:onShowGameList(sender, eventtype)
    ...
    if eventtype == ccui.TouchEventType.ended then
        laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SENDRENMAN_GOGAMELIST)
        self:destroy()
    end
end

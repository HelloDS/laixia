...
function TPBankruptcyPackage:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_BANKRUPTBAG_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SEND_BANKRUPTBAG_WINDOW, handler(self, self.sendRequest))
end

function TPBankruptcyPackage:sendRequest()
    local CSBankruptBag = Packet.new("CSBankruptBag", _LAIXIA_PACKET_CS_BankruptBagID)
    CSBankruptBag:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)
...
function TPBankruptcyPackage:onShow(msg)
    ...
    self.mItemID = laixiaddz.LocalPlayercfg.LaixiaBankruptItemID
    self:showDetail()
end
...
function TPBankruptcyPackage:showDetail()
    ...
    local goods = laixiaddz.JsonTxtData:queryTable("shop"):query("itemsID",self.mItemID)
    local price = 0 

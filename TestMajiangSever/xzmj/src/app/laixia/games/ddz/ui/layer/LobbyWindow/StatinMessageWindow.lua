...
function StatinMessage:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_MAIL_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_MAIL_WINDOW,handler(self,self.updateWindow))
end
...
function StatinMessage:onShow()
    if not self.mIsShow then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDES_LETTERRED_WINDOW) --隐藏红点
...
function StatinMessage:onClearup(msg)
    userDefault:setIntegerForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "number", 0)
...
function StatinMessage:saveLetter()
    for i, v in ipairs(self.letterArray) do
        if i <= 30 then
            userDefault:setStringForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "sendTime" .. i,v.sendTime)
            userDefault:setStringForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "conText" .. i,v.Context)
        end
    end
    userDefault:setIntegerForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "number", #self.letterArray)
end
...
function StatinMessage:getLetterArray()
    local number = userDefault:getIntegerForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "number")
    if number ~= nil and number > 0 then
        for i = 1, number do
            if laixiaddz.LocalPlayercfg.LaixiaHeartBeatTime == 0 then
                local heartTime = userDefault:getStringForKey("heartTime")
                if heartTime~=nil then
                    laixiaddz.LocalPlayercfg.LaixiaHeartBeatTime = tonumber(heartTime)
                end
            end
            local cha = (laixiaddz.LocalPlayercfg.LaixiaHeartBeatTime/1000) - tonumber(userDefault:getStringForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "sendTime" .. i))
            if cha < (laixia.config.LAIXIA_SHOW_STATINMASSAGE*24*3600) then    --7天等于518400
                local letter = { }
                letter["sendTime"] = userDefault:getStringForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "sendTime" .. i)
                letter["Context"] = userDefault:getStringForKey(laixiaddz.LocalPlayercfg.LaixiaPlayerID .. "conText" .. i)

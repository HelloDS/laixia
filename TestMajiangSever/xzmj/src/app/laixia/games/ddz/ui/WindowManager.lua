...
--    self:doPreLoad();
    ...
    -- ---------------------------------------
    -- self:doLoad();
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_RETURN_BACK, handler(self, self.onCallBackFunction));
...
--        :addItem("PokerDesk.AppleDismissLayer")
        ...
        :addItem("layer.LobbyWindow.GameSetsWindow")-- 设置窗口
        :addItem("layer.LobbyWindow.GameAbout")-- 关于窗口
...
function WindowManager:updateZOrder()
    local mZorder=100
    if (self.mTopWindow ~= nil) then
        self.mTopWindow.root:setLocalZOrder(1000+mZorder)
        mZorder=mZorder+1
        if self.mWindowListByInner[self.mTopWindow:getName()] then
            for _,v in ipairs(self.mWindowListByInner[self.mTopWindow:getName()]) do
                v.root:setLocalZOrder(1000+mZorder)
                mZorder=mZorder+1
            end
        end
    end
    for k,v in ipairs(self.mWindowListByNormal) do
        ...
        v.root:setLocalZOrder(1000+mZorder + zorder)
        mZorder=mZorder+1

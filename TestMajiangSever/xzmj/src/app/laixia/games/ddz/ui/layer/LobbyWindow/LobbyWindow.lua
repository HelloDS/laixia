...
------------------------------------------------------------------------------------------------
    ...
    if laixia.config.isAudit then
        ...
        elseif laixiaddz.kconfig.isYingKe == false then
        ...
        if self:isHasTaskRedPack() and laixiaddz.kconfig.isYingKe == false then
...
--     }
     ...
     -- end
        ...
        if laixiaddz.kconfig.isYingKe == false then
...
function LobbyWindow:showBagTips()
    if self.mIsLoad and laixiaddz.kconfig.isYingKe == false then
        self.Image_tips:setVisible(true)
    end   
end
    ...
    if not laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData then
       ...
       cc.UserDefault:getInstance():setDoubleForKey("sign_time",os.time())
        ...
        if tonumber(os.time()) - tonumber(reSign_time) >= 60*60 then
           self:sendActivity()
           cc.UserDefault:getInstance():setDoubleForKey("sign_time",os.time())
        elseif laixiaddz.LocalPlayercfg.isShouchong == true and laixiaddz.kconfig.isYingKe == false then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FIRSTGIFT_WINDOW)
        end
    end
...
function LobbyWindow:onTick(dt)
    ...
    if self.mIsShow then
       ...
       --  end
        ...
        if laixiaddz.LocalPlayercfg.isShouchong == false or laixiaddz.kconfig.isYingKe == true then
            self.Button_Lobby_ShouChong:setVisible(false)
            if laixia.config.isAudit == false and laixiaddz.kconfig.isYingKe == false then
                self.chest:setVisible(false)
            end
        end
         --排行榜

...
function ExchangeCode:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_REDEEMGIFT_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_REDEEMGIFT_WINDOW, handler(self, self.destroy))
end
...
function ExchangeCode:onSendToSever(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if RedeemCode == "" then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "请输入兑换码")
            return
        end
        local CSRedeemCode = Packet.new("CSRedeemCode", _LAIXIA_PACKET_CS_RedeemCodeID)
        CSRedeemCode:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)

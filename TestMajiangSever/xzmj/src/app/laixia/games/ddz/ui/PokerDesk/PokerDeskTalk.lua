...
function PokerDeskTalk:sendsever(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "CardTableDialog" then
            ...
            stream:setValue("type", talk_type)
            stream:setValue("info", str)
...
function PokerDeskTalk:getpatch(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "CardTableDialog" then
            ...
            stream:setValue("room_id", ui.CardTableDialog.mRoomID)
            stream:setValue("table_id", ui.CardTableDialog.hDeskID)
            stream:setValue("type", 0)
            stream:setValue("info", self.expression_num)
            stream:setValue("msg_id","table_chat")
            laixiaddz.net:sendSocketPacket(stream)
        end
        local mesg = { }
...
function PokerDeskTalk:onInputText(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        stream:setValue("table_id", ui.CardTableDialog.hDeskID)
        stream:setValue("type", 1)
        stream:setValue("info", self:GetWidgetByName("TextField_Content"):getString())
        stream:setValue("msg_id","table_chat")
        laixiaddz.net:sendSocketPacket(stream)
    end
end

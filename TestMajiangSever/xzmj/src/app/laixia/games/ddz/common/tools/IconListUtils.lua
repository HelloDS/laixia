...
function IconListUtils.clickGameIcon(clickItem)
    ...
    if clickItem and clickItem.m_gameInfo then
        ...
        if gameId == "1" then
            IconListUtils.rotateGameView(false)
...
--转屏幕 
--android true转换横屏
function IconListUtils.rotateGameView(landscapeData)
    if device.platform == "android" then
        luaj.callStaticMethod("com/laixia/game/ddz/AppActivity", "rotate", {landscapeData}, "(Z)V")
    elseif device.platform == "ios" then
        luaoc.callStaticMethod("IKCRLXBridgeManager", "changeToLandscape", {landscape = landscapeData})
    end
end

require "app.laixia.game.anysdkConst"
...
function PluginChannel:onUserResult(code, msg )
    ...
    elseif code == UserActionResultCode.kLoginSuccess then --登录成功
        ...
        local USER_ID=data[1]
        local TOKEN=data[2]
        local HEADIMAGE = data[3] or ""
        local NICKNAME = data[4] or ("用户"..os.time())
        -------------------
        laixia.LocalPlayercfg.LaixiaUserID = USER_ID
        print("laixia.LocalPlayercfg.LaixiaUserID"..laixia.LocalPlayercfg.LaixiaUserID)
        laixia.LocalPlayercfg.LaixiaTokenID = TOKEN
        print("laixia.LocalPlayercfg.LaixiaTokenID"..laixia.LocalPlayercfg.LaixiaTokenID)
        --laixia.LocalPlayercfg.LaixiaPlayerNickname = respJson.nickName
        laixia.LocalPlayercfg.HEAD_URL = HEADIMAGE
        print("laixia.LocalPlayercfg.NICKNAME"..NICKNAME)
        laixia.LocalPlayercfg.newPassWord = NICKNAME
        print("laixia.LocalPlayercfg.HEAD_URL"..laixia.LocalPlayercfg.HEAD_URL)
        --laixia.LocalPlayercfg.LaixiaPlayerGender = respJson.sex - 1
        if USER_ID~="" then
            cc.UserDefault:getInstance():setStringForKey("uid", USER_ID)
        end
        if TOKEN~="" then
            cc.UserDefault:getInstance():setStringForKey("token", TOKEN)
        end
        ---- cc.UserDefault:getInstance():setStringForKey("unionid", respJson.unionid)
        if HEADIMAGE~="" then
            cc.UserDefault:getInstance():setStringForKey("headimgurl",HEADIMAGE)
        end
        cc.UserDefault:getInstance():setBoolForKey("isauto", true)

        laixia.config.GamePlatformID = 10
        cc.UserDefault:getInstance():setIntegerForKey("GamePlatformID", 10)
        laixia.LocalPlayercfg.LaixiaLastLoginPlatform = 10
...
function PluginChannel:ctor()
    ...
    if laixia.kconfig.isYingKe ~=  true then
        ...
        local oauthLoginServer = "http://laixia.f3322.net:11121/laixia-hall/login.do";
    ...
    if laixia.kconfig.isYingKe==true then
        laixia.LocalPlayercfg.CHANNELID = 201010
    else
        laixia.LocalPlayercfg.CHANNELID = agent:getChannelId();
    end
...
function PluginChannel:pay(productType,ProductId,rmb,produceName,orderID)
    ...
    if iap_plugin_maps ~= nil then
        local info = {
            ...
            Server_Id="1",
            Product_Count="1",
            Role_Id=laixia.LocalPlayercfg.LaixiaUserID,
            Role_Name="asd",
            EXT = orderID
        }

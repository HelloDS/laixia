...
function ObjectLocalPlayerData:getAccountID()
    ...
    elseif ObjectLocalPlayerData.LaixiaLastLoginPlatform == 10 then
        rcValue = ObjectLocalPlayerData.LaixiaUserID;
    else
...
function ObjectLocalPlayerData.getumengChannelID()    
	...
	elseif device.platform == "android" then
	    ...
	    local state, value = luaj.callStaticMethod(javaClassName, javaMethodName, javaParams, javaMethodSig)
	    rcValue = value 
        if tonumber(rcValue)<100000 then
            rcValue = string.format("%06d",rcValue)
        end
    elseif device.platform == "ios" then
...
ObjectLocalPlayerData.LaixiaMatchSigndata = {} --记录标签信息的数组  --byfeng
...
ObjectLocalPlayerData.LaixiaisLIUJU = false   -- 是否流局
-- ObjectLocalPlayerData.LaixiaisShiMing = false   -- 是否实名认证(false表示没有实名，true表示已实名)

--活动
ObjectLocalPlayerData.LaixiaisActivity = false  --判断是否请求下载活动图
ObjectLocalPlayerData.LaixiaActivityPath = {} --活动图路径
ObjectLocalPlayerData.LaixiaLunBoPath = {}    --轮播图路径
ObjectLocalPlayerData.LaixiaLunBoName = {}    --轮播图名
ObjectLocalPlayerData.LaixiaisDownload = false   --轮播图
ObjectLocalPlayerData.LaixiaActivityName = nil  -- 活动名

...
function PCBindPhone:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_BINDPHONE_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_BINDPHONE_WINDOW, handler(self, self.destroy))
end
...
function PCBindPhone:sendBindPacket(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        CSBindingPhone:setValue("Code",laixia.LocalPlayercfg.LaixiaHttpCode)
        ...
        laixia.LocalPlayercfg.LaixiaPhoneNumTMP = self.PhoneNumber:getText()
        laixia.LocalPlayercfg.LaixiaPasswordTMP = ""
...
function PCBindPhone:getPhoneCode(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
       ...
       if string.len(phonenumber) < 11  or tonumber(phonenumber) ==nil then
        ...
        CSCodeBinding:setValue("Code",laixia.LocalPlayercfg.LaixiaHttpCode) 

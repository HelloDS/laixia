...
function PCChangePassWord:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_REVISEPASSWD_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_REVISEPASSWD_WINDOW, handler(self, self.destroy))
end
...
function PCChangePassWord:onSendToSever(sender,event)
    if event == ccui.TouchEventType.ended then
        ...
        if self.Old_PassWord:getString() == ""  then
           ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,"请输入当前密码！")
           return
        end
        ...
        if  flag1 == false or  string.len(NewPw_2)<6 or string.len(NewPw_2)>18  then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "密码无效，请输入6~18位数字、字母或下划线组合")
            return
        end
        if NewPw_2 == "" then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,"请再次输入密码")
            return
        end
        if NewPw_2 ~= NewPw_1 then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,"两次密码不一致")
            return
        end

        local CSRevisePwd = Packet.new("CSRevisePwd",_LAIXIA_PACKET_CS_RevisePwdID)
        CSRevisePwd:setValue("Code",laixia.LocalPlayercfg.LaixiaHttpCode)
        ...
        laixia.LocalPlayercfg.LaixiaPasswordTMP = self.New_Password_2:getString();

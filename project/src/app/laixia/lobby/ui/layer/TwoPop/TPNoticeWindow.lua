...
function TPNoticeWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_BULLETINS_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_UPDATE_BULLETINS_WINDOW, handler(self, self.updateWindow))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_REQUEST_BULLETINS_WINDOW, handler(self, self.requestNotice))
end

function TPNoticeWindow:requestNotice()
    local CSServiceNotice = Packet.new("CSServiceNotice", _LAIXIA_PACKET_CS_ServiceNoticeID)
    CSServiceNotice:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)

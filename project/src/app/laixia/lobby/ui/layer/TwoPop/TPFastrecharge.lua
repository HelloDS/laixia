...
function TPFastrecharge:onInit()
    ...
    if(4 == Platform  or 5 == Platform ) then
        ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_INCREASEGOLD_WINDOW, handler(self, self.iosFun))
    else
        ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_INCREASEGOLD_WINDOW, handler(self, self.show))
	end

    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_INCREASEGOLD_WINDOW, handler(self, self.destroy))
end

function TPFastrecharge:iosFun()
   --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,"金币不够浪了，请充值再战！")
   ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,{Text= "金币不够浪了，请充值再战！ " , OnCallFunc= self.shutDownFun })
...
function TPFastrecharge:showCoinInfo()
    ...
    local goods = laixia.JsonTxtData:queryTable("shop").buf
...
function TPFastrecharge:buyCoin(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        laixia.LocalPlayercfg.LaixiaIsShowPayWindow =  false
        laixia.LocalPlayercfg.LaixiaPayType = sender.mType
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_ORDERINFO_WINDOW,{id=itemId})
    end
end
...
function TPFastrecharge:onShow(data)
   ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_SELFBUILDING_WINDOW) --关闭自建桌

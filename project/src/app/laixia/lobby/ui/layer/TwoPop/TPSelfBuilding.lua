...
function TPSelfBuilding:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_SELFBUILDING_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_SELFBUILDING_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_PACKET_CREATESELFBUILF, handler(self, self.onSendCreateList))
end
...
function TPSelfBuilding:onShow(msg)
    ...
    self:GetWidgetByName("Text_RoomName"):setString(laixia.LocalPlayercfg.LaixiaPlayerNickname.."房间")
...
function TPSelfBuilding:onJoinGame(sender, eventType)
    --if eventType == ccui.TouchEventType.ended then
        ...
        local tableID = tonumber(str)
        if str == "" then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "需要输入房号！")
            self:onResetNum()
            return
        end
        if tableID ~= 0 then

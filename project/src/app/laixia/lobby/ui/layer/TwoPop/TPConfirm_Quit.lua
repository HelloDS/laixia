...
function Confirm_Quit:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_ANDROIDBACKTK_WINDOW, handler(self, self.show))    
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_EXITGAME_WINDOW, handler(self, self.exitGame))
end
...
function Confirm_Quit:onGetCoin(sender,eventType)
    if(eventType == ccui.TouchEventType.ended) then
        ...
        if laixia.LocalPlayercfg.LaixiaCurrentWindow ~= "CardTableDialog"then
            if laixia.LocalPlayercfg.LaixiaCurrentWindow == "GameRoomGround" and ui.GameRoomGround.mRoomType ~= nil then
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_QUICKGAME_WINDOW,ui.GameRoomGround.mRoomType)
            else

...
function TPPayWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_PAY_WINDOW,handler(self,self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_PAY_TOFUN,handler(self,self.LaixiaSDkPay))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_PAY_WEIYOUXI,handler(self,self.WeiyouxiPay))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_PAY_XIAOMI,handler(self,self.XiaomiPay))
...
function TPPayWindow:XiaomiPay(msg)
    if msg and msg.data and msg.data.orderID then
        ...
        local mShopItem = laixia.JsonTxtData:queryTable("shop"):queryMallInfoByID(msg.data.ItemID);
    ...
    if (cc.PLATFORM_OS_ANDROID == targetPlatform) then
        ...
        local function callback(isSuccess)
            if isSuccess==true then
                ...
                --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_EFFECTGOLD_WINDOW,{iconpath=iconpath})
                --     return
                -- end
                ...
                -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"充值失败")
            end
        end
        laixia.LocalPlayercfg.PluginChannel:setEndCallBack(callback)
        laixia.LocalPlayercfg.PluginChannel:pay(channelName,args.ItemID,args.Price,args.productName,args.orderID)
    end   
end
...
local function sendOrderToSever(msg)
    ...
    onsendtoSever:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
...
function TPPayWindow:onIOSPay(msg)
    ...
    if msg and msg.data and msg.data.orderID then
        ...
        local mShopItem = laixia.JsonTxtData:queryTable("shop"):queryMallInfoByID(tostring(msg.data.ItemID));
        ...
        if(item ==nil) then
            ...
            payType = laixia.LocalPlayercfg.LaixiaPayType --1  表示是微信 2 是支付宝
            ...
            -- Android_SDK(args)
            -- laixia.LocalPlayercfg.LaixiaPayType = 0
            -- laixia.LocalPlayercfg.LaixiaIsShowPayWindow =  true
    end
    print(laixia.LocalPlayercfg.CHANNELID)
    print(payType)
    print(msg.data.ItemID)
    print(Price)
    print(item.ItemName)
    ...
    if tostring(laixia.LocalPlayercfg.CHANNELID) == "500026" then --苹果官网
        ...
        laixia.LocalPlayercfg.PluginChannel:setEndCallBack(callback)
        laixia.LocalPlayercfg.PluginChannel:pay("appstore",msg.data.ItemID,Price,item.ItemName,orderID)
    ...
    elseif tostring(laixia.LocalPlayercfg.CHANNELID) == "111267" or tonumber(laixia.LocalPlayercfg.CHANNELID)>200000 then --微信 自己官网版本 有微信
        local channelName = 0
        if payType == 1 then
            channelName = "weixin"
        ...
        print("111267 pay")
        laixia.LocalPlayercfg.PluginChannel:setEndCallBack(callback)
        laixia.LocalPlayercfg.PluginChannel:pay(channelName,msg.data.ItemID,Price,item.ItemName,orderID)
        laixia.LocalPlayercfg.LaixiaPayType = payType
        laixia.LocalPlayercfg.LaixiaIsShowPayWindow =  true
    end
end
...
function TPPayWindow:onAndroldPay(msg)
    ...
    if msg and msg.data and msg.data.orderID then
        ...
        if(item ==nil) then
            ...
            local ItemName = item.ItemName;
            local payType = laixia.LocalPlayercfg.LaixiaPayType --1  表示是微信 2 是支付宝
            local AppType =  laixia.config.GameAppID --游戏类型
            local length = string.len(tostring(orderID))
            if length < 16 then
                local num = 16-length
...
function TPPayWindow:LaixiaSDkPay(msg)
    ...
    -- local mShopItem = laixia.JsonTxtData:queryTable("shop"):queryMallInfoByID(tostring(ItemID));
    -- local Price = mShopItem["price"]
    -- local targetPlatform = cc.Application:getInstance():getTargetPlatform()
...
function TPPayWindow:goPay(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
       ...
       local msg = self.msg
       laixia.LocalPlayercfg.LaixiaPayType = tag

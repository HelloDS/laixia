...
function TiShiyuConnectCircle:ctor(...)
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_RECONNECTIONTIPS_WINDOW, handler(self, self.showConnectCircleAni))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW, handler(self, self.destroyConnectCircleAni))
...
function TiShiyuConnectCircle:tick(dt)
    ...
    if not self.showOne then -- 防止出现的太频繁
        if (laixia.LocalPlayercfg.LaixiaCurrentWindow ~= "CardTableDialog" and self.mTimer >= self.mTimeByShow) or
            (laixia.LocalPlayercfg.LaixiaCurrentWindow == "CardTableDialog" and self.mTimer >= self.mTimeByShow1) then
            self.showOne = true
    ...
    elseif (self.mTimer > self.mTimeout) then
        ...
        if (waitingType == EnumWaitingType.EWT_NORMAL) then
            laixia.helper.popupReLoginWindow(laixia.utilscfg.DICT(_ID_DICT_TYPE_CONNECT_TIMEOUT), "http")
        elseif (waitingType == EnumWaitingType.EWT_NET_HTTP) then
            laixia.helper.popupReLoginWindow(laixia.utilscfg.DICT(_ID_DICT_TYPE_CONNECT_ERROR), "http")
        ...
        elseif waitingType == EnumWaitingType.EWT_ON_SUBAPP then
            laixia.helper.popupReLoginWindow(laixia.utilscfg.DICT(_ID_DICT_TYPE_CONNECT_TIMEOUT), "http")
            laixia.helper.failedLoadSubApp();

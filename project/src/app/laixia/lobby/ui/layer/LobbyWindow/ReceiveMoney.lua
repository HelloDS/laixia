...
function ReceiveMoney:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MONEY_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_HIDEMONEY_WINDOW, handler(self, self.destroy))
end
...
function ReceiveMoney:onShutDown(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_HIDEMONEY_WINDOW)
    end
end

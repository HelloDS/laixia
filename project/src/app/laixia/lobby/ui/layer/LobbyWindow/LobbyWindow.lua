...
local IconListUtils = require("lobby.ui.tools.IconListUtils")
...
function LobbyWindow:onShow()
    ...
    app.m_mainLayer = self.mInterfaceRes
...
------------------------------------------------------------------------------------------------
    ...
    self.mInterfaceRes = self:GetWidgetByName("panel_root")
    -- 首先载入
    self.LobbyTop = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_center_top")
    self.panel_center = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_center")
    self.panel_right_middle = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_right_middle")
    self.mainPanel = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_center_middle")
    self.Button_Lobby_Dayly = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_sign")
    self.Button_Lobby_TurnTable = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_zhuanpan")
    self.Button_Lobby_ShouChong = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_shouchong")
    self.bottom = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_center_bottom")
    self.bottom_right = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"panel_right_bottom")
    
     --创建游戏品类图标 by wangtianye
    self:createGridNode()
...
--            self.sign:pos(self.Button_Lobby_Dayly:getPositionX()-8,self.Button_Lobby_Dayly:getPositionY()-2)
            ...
            --幸运转盘
            self.LobbyRight = self:GetWidgetByName("panel_right")
            self.LobbyRight_top = self:GetWidgetByName("panel_right_top")
            self:addButAnimationEventListener(self.Button_Lobby_TurnTable,handler(self, self.onGoTurnTable))
            self.Button_Lobby_TurnTable:setOpacity(0)
            local system2 = laixia.ani.CocosAnimManager
            self.turntable = system2:playAnimationAt(self.LobbyRight_top,"doudizhu_icon_turntable")
            self.turntable:pos(self.Button_Lobby_TurnTable:getPositionX(),self.Button_Lobby_TurnTable:getPositionY())
            --首充
            self.Button_Lobby_ShouChong:setOpacity(0)
            local system3 = laixia.ani.CocosAnimManager
            self.chest = system3:playAnimationAt(self.LobbyRight_top,"doudizhu_icon_chest")
...
--        self:addButAnimationEventListener(self:GetWidgetByName("Button_Renwu",self.bottom),handler(self, self.onRenwu))
        -- 背包的监听放在这
        self.btBackpack=self:GetWidgetByName("btn_bag",self.bottom)
...
--        end
        ...
        self:addButAnimationEventListener(self.Button_Lobby_Dayly,handler(self, self.onShowDayley))
        self:addButAnimationEventListener(self.Button_Lobby_ShouChong,handler(self, self.onShouchong))
...
--        self:addButAnimationEventListener(self.Button_bisaichang,handler(self,self.gotoBisaichang))
        ...
        self.AdvertisementPageView = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"pgv_pic")
...
--        self.btEail = self:GetWidgetByName("Button_Mail",self.bottom)
        self.btEail = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_mail")
        self:addButAnimationEventListener(self.btEail,handler(self, self.sendStatinMessagePacket))
        

        local mActivity = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_activity")
--        local mActivity = self:GetWidgetByName("Button_Activity",self.bottom)
        ...
        self.Button_Setting = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_set")
        self:addButAnimationEventListener(self.Button_Setting,handler(self, self.GameSets))


        if laixia.kconfig.isYingKe == false then
            self.mShoping = ccui.Helper:seekWidgetByName(self.mInterfaceRes,"btn_shop")--self:GetWidgetByName("Button_Shop",self.bottom)
            self.mShoping:setOpacity(0)

            local system = laixia.ani.CocosAnimManager
            self.shoppingAni = system:playAnimationAt(self.bottom_right,"doudizhu_shopping")
        ...
        --排行
        self.mRanking = self:GetWidgetByName("btn_rank",self.bottom)
...
--        self:GetWidgetByName("Button_youxichang",self.mainPanel):setVisible(false)
        ...
        self.panel_left = self:GetWidgetByName("panel_left",self.mInterfaceRes)
        self.panel_left_top = self:GetWidgetByName("panel_left_top", self.panel_left)
        self:addButAnimationEventListener(self:GetWidgetByName("img_head",self.panel_left_top),handler(self, self.sendPersonalCenterPacket))

        self.Whao = self:GetWidgetByName("btn_go_wenhao",self.panel_left)
        ...
        --self.top = ccui.Helper:seekNodeByName(self.mInterfaceRes,"panel_left")
        self.Button_shoujibangding = self:GetWidgetByName("btn_phone",self.panel_left_top)--self:GetWidgetByName("Button_shoujibangding",self.top)
--        local system4 = laixia.ani.CocosAnimManager
--        self.phone = system4:playAnimationAt(self.panel_left_top,"doudizhu_phone")
...
--        self:addButAnimationEventListener(self.phone,handler(self, self.shoujibangding))
...
function LobbyWindow:createGridNode()
...
--        self.pv:reload()  
    ...
    local pageHolder = self:GetWidgetByName("panel_gamelist")
    pageHolder:setLocalZOrder(1000)
    pageHolder:setAnchorPoint(cc.p(0.5,1))
    local size = pageHolder:getContentSize()
    local gridNode = require("lobby.ui.node.GridNode").new({
        column = 4,
        row = 2,
        columnSpace = 0,
        rowSpace = 0,
...
function LobbyWindow:onGridListener(event)
...
function LobbyWindow:setIconListInfo(gridNode, gameList)
...
--            :align(display.CENTER, IconListUtils.m_itemSize.width / 2, IconListUtils.m_itemSize.height / 2)
...
function LobbyWindow:addPage(pIdx, iIdx, bClone)
    ...
    local panel_picture = self:GetWidgetByName("pgv_pic",self.AdvertisementPageView)--self:GetWidgetByName("img_picture",newPage)
    local adImg = self:GetWidgetByName("img_picture",panel_picture)
...
function LobbyWindow:setTopInfo()
    if laixia.LocalPlayercfg.LaixiaCurrentWindow ==  "LobbyWindow" then
        local temp = self:GetWidgetByName("img_gold_dikuang",self.panel_left_top)
        self.AtlasLabel_Lable_Gold_Number = self:GetWidgetByName("txt_gold",temp)
        self.AtlasLabel_Lable_Gold_Number:setString(laixia.LocalPlayercfg.LaixiaPlayerGold)
--        self:GetWidgetByName("AtlasLabel_Lable_Gold_Number",self.top):setString(laixia.LocalPlayercfg.LaixiaPlayerGold)
        local temp = self:GetWidgetByName("img_laidou_dikuang",self.panel_left_top)
        self.AtlasLabel_Lable_JiangQuan_Number = self:GetWidgetByName("txt_laidou",temp)
        self.AtlasLabel_Lable_JiangQuan_Number:setString(laixia.LocalPlayercfg.LaixiaPlayerGiftCoupon)
--        self:GetWidgetByName("AtlasLabel_Lable_JiangQuan_Number",self.top):setString(laixia.LocalPlayercfg.LaixiaPlayerGiftCoupon)
        local temp = self:GetWidgetByName("img_head_bg",self.panel_left_top)
        self.nickName = self:GetWidgetByName("txt_playername",temp)
        self.nickName:setString(laixia.helper.StringRules_6(laixia.LocalPlayercfg.LaixiaPlayerNickname))
--        self:GetWidgetByName("nickName",self.top):setString(laixia.helper.StringRules_6(laixia.LocalPlayercfg.LaixiaPlayerNickname)) 
        ...
        -- else
            ...
            self.Button_shoujibangding:setOpacity(0)
...
--                self.phone:setVisible(false)
                  self.Button_shoujibangding:setVisible(false)
            end
--        end
...
--            self.phone:pos(self.Button_shoujibangding:getPositionX(),self.Button_shoujibangding:getPositionY())
            self.Button_shoujibangding:pos(self.Button_shoujibangding:getPositionX(),self.Button_shoujibangding:getPositionY())
            -- 头像
...
--    end
         if laixia.config.isAudit then
             table.insert(self.mPathArray,"new_ui/isAudit/rechargefuli_guanggao.png")
             table.insert(self.mPathArray,"new_ui/isAudit/rechargefuli_guanggao.png")
             table.insert(self.mPathArray,"new_ui/isAudit/rechargefuli_guanggao.png")
             table.insert(self.mPathArray,"new_ui/isAudit/rechargefuli_guanggao.png")
             table.insert(self.mPathArray,"new_ui/isAudit/rechargefuli_guanggao.png")
             ...
             table.insert(self.mPathArray,"new_ui/lobbywindow/dajiangsai_guanggao.png")
             table.insert(self.mPathArray,"new_ui/lobbywindow/dajiangsai_guanggao.png")
             table.insert(self.mPathArray,"new_ui/lobbywindow/dajiangsai_guanggao.png")
             table.insert(self.mPathArray,"new_ui/lobbywindow/dajiangsai_guanggao.png")
             table.insert(self.mPathArray,"new_ui/lobbywindow/dajiangsai_guanggao.png")
...
function LobbyWindow:onTick(dt)
    ...
    if self.mIsShow then
        ...
        if m_lunboTime >= 2 and m_lunboTime <= 2.03 then--and self.isDiaoYong ~= true then
            self:addPathArray()
            self:updatePageView()
        end
       -- if laixia.kconfig.isYingKe == true then
...
--        DownloadActivity:tick()
        ...
        if laixia.LocalPlayercfg.LaixiaPlayerGold ~=nil and laixia.LocalPlayercfg.LaixiaPlayerGold> 0 then
            ...
            self.AtlasLabel_Lable_Gold_Number:setString(laixia.helper.numeralRules_2(laixia.LocalPlayercfg.LaixiaPlayerGold))
        end

...
--    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_BROADCASTS_WINDOW, handler(self, self.show))  
    else
        ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_BROADCASTS_WINDOW, handler(self, self.show)) 
    end
end
...
function BroadcastWindows:resetData()
    laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoom = {} 
    laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomCoupon = {} 
    laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomMatch = {} 
...
function BroadcastWindows:CutNode(data)
    ...
    local function showPaomadeng()
            ...
            if info.show == 100 then
                ...
                laixia.LocalPlayercfg.TurnWindow_NickName = data.text
                laixia.LocalPlayercfg.TurnWindow_Award = data.award

                cc.UserDefault:getInstance():setStringForKey("turn_name", laixia.LocalPlayercfg.TurnWindow_NickName)
                cc.UserDefault:getInstance():setStringForKey("turn_award", laixia.LocalPlayercfg.TurnWindow_Award)
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_REFRESH_TOTURNTABLE_PRICEINFO,data)
            end
...
function BroadcastWindows:getCurrentMsgData()
    ...
    if laixia.LocalPlayercfg.LaixiaCurrentWindow == "CardTableDialog" then 
        ...
        if ui.CardTableDialog.mPokerDeskType == 1 then  
            ...
            if #laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoom >0 then 
                self.currentShowType=2
                info = laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoom[1] 
                table.remove(laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoom,1)
        ...
        elseif ui.CardTableDialog.mPokerDeskType == 2 then 
            ...
            if #laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomCoupon >0 then 
                self.currentShowType=3
                info = laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomCoupon[1] 
                table.remove(laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomCoupon,1)
                self.delayTime = data.Num 
            end
        elseif ui.CardTableDialog.mPokerDeskType == 3 then  
            ...
            if #laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomMatch >0 then
                self.currentShowType= 4
                info = laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomMatch[1] 
                table.remove(laixia.LocalPlayercfg.LaixiaBroadCastInPokerRoomMatch,1)
                self.delayTime = data.Num 
            end
        else 
            return nil 
        ...
        if #laixia.LocalPlayercfg.LaixiaBroadCastInInHall >0 then 
           self.currentShowType = 6 
           info = laixia.LocalPlayercfg.LaixiaBroadCastInInHall[1] 
           table.remove(laixia.LocalPlayercfg.LaixiaBroadCastInInHall,1)           
           self.delayTime = data.Num 
        end
    end 
    return info 

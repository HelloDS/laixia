...
local laixia = laixia;  --
local LoadingWindow = class("LoadingWindow", import(".CSBBase"):new())-- 
    ...
    if laixia.kconfig.isYingKe == true then
        ...
        --if device.platform == "ios" then
          --  local luaoc = require("cocos.cocos2d.luaoc")
            --- local state,value = luaoc.callStaticMethod("lk", "getAppVersionCode");
       -- elseif device.platform == "android" then
       --     local javaClassName = APP_ACTIVITY
        ...
        -- cc.UserDefault:getInstance():setStringForKey("uid", laixia.LocalPlayercfg.LaixiaUserID)
...
--映客登录
function LoadingWindow:showYKLogin()
...
function LoadingWindow:showWXLogin()
    ...
    if laixia.LocalPlayercfg.CHANNELID~=nil and ((tostring(laixia.LocalPlayercfg.CHANNELID) == "160186" or tostring(laixia.LocalPlayercfg.CHANNELID) == "111267") or (tonumber(laixia.LocalPlayercfg.CHANNELID)>200000 and tonumber(laixia.LocalPlayercfg.CHANNELID)<300000)) then
        if laixia.kconfig.isYingKe == true then
            ...
            self.wxLogin:setPositionX(display.cx)
            self.entry:setPositionX(self.wxLogin:getPositionX())
...
function LoadingWindow:setAdaptation()
    ...
    -- self.zs_ButtonBack:setVisible(true)
    --修2
...
function LoadingWindow:onShow(data)
    ...
    if laixia.kconfig.isYingKe == true then
        ...
        if device.platform == "windows" then
            ...
            laixia.LocalPlayercfg.PluginChannel =require("common.game.PluginChannel").create()
...
function LoadingWindow:isTransition()
...
function LoadingWindow:getVersion(data)
    ...
    if data.data.StatusID~=0 then
        local RemoteVersion = data.data.Version

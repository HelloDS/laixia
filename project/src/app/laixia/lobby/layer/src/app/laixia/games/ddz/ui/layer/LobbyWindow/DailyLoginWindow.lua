...
function DailyLoginWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_DAILYSIGN_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_DAILYSIGN_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SIGNIN_DAILYSIGN_WINDOW, handler(self, self.onShowSign))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SEND_QUICKBAG_WINDOW, handler(self, self.sendSuperBagPacket))
end
...
function DailyLoginWindow:sendSuperBagPacket()
    ...
    CSQuickSuperBag:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)
...
function DailyLoginWindow:onShowUI(dataArray ,TypeInfo)
   ...
   for i, v in ipairs(dataArray) do
        ...
        -------签到
        if i < laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.SignDay then 
        ...
        if i == laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.SignDay then 
            if  laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.IsSign == 1 then   --签过了
                self.ReceiveIcon = self:GetWidgetByName("Image_Received_Icon",node_Day) 
...
--        local sign = false 
--        if i == laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.SignDay then 
--            if  laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.IsSign == 1 then   
--                sign = true 
--            else 
--                self.ImgLight = self:GetWidgetByName("Image_Everyday_light",node_Day)
--                self.ImgLight:setVisible(true)
...
--    self.mBTshutDown :setVisible(false)
   ...
   self:onShowUI(laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.Shows , laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.TypeSign)
...
function DailyLoginWindow:onShowSign(data)
    if (not self.mIsLoad ) then
        if laixiaddz.LocalPlayercfg.LaixiaShowActivyWindow == 1 then 
    ...
    if self.Today ~= nil then 
        -- 签到获取金币的动画
         local iconpath = "new_ui/everydaylogin/day"..tonumber(laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.SignDay)..".png"
         local jinbiNum
        ...
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_EFFECTGOLD_WINDOW,{iconpath=iconpath,jinbiNum=jinbiNum})
--        self.ReceiveMenban:setVisible(true)


        local node_Name = "Image_Everyday_0" .. (laixiaddz.LocalPlayercfg.LaixiaContinuousLoginData.SignDay)
        local node_Day = self:GetWidgetByName(node_Name)
...
function DailyLoginWindow:onDestroy()
    if laixiaddz.LocalPlayercfg.LaixiaIsSign == 1 then 
       -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_ACTIVITY_WINDOW)

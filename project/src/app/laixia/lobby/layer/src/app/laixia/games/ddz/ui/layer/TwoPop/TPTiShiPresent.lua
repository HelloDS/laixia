...
function TPTiShiPresent:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_PRESENT_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_PRESENT_WINDOW, handler(self, self.destroy))
end
...
function TPTiShiPresent:onAdd(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if num > self.mMax then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"没有更多物品可赠送") 
        elseif num >= 999 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"最多只能赠送999个物品")  
        else
            num = num +1
...
function TPTiShiPresent:onMinus(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        num = num - 1
        if num <= 0 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"赠送物品数量不能为空") 
        else
            self.mPresentNum:setString(num)
        end
    end
      ...
      if playerID == 0  or playerID == nil  then
         ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "请输入有效的玩家ID") 
          ...
          local mPresent = Packet.new("mPresent", _LAIXIA_PACKET_CS_PresentID)
            mPresent:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)

...
function ExchangeItemDetails:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_REDEEMDETAILS_WINDOW, handler(self, self.show))
end
...
function ExchangeItemDetails:onExchange(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)
        if self.exchangeInfo.ItemCount > laixiaddz.LocalPlayercfg.LaixiaPlayerGiftCoupon then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "奖券不足玩牌可以获取更多的奖券！")
            return
        end
        if self.exchangeInfo.ItemNum==0 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "今天已经换光啦，明天再来看看吧")
            return
        end
        local dataTable = { }
        ...
        if self.Itemsdata.delivery  == 1 then
            ObjectEventDispatch:dispatchEvent( { name = _laixiaddz_EVENT_SHOW_REDEEMENTITY_WINDOW, data = dataTable})
        elseif self.Itemsdata.delivery  == 2  then
            self:onSendPacketToSever(dataTable)
        else
            ObjectEventDispatch:dispatchEvent( { name = _laixiaddz_EVENT_SHOW_REDEEMVIRTUAL_WINDOW, data = dataTable})
        end
        self:destroy()
    end
end
...
function ExchangeItemDetails:onShow(mesginfo)
    ...
    self:GetWidgetByName("ED_Label_Assets"):setString(laixiaddz.LocalPlayercfg.LaixiaPlayerGiftCoupon .. "奖券")

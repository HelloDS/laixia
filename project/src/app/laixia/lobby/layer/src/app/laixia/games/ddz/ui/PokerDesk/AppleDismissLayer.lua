...
function AppleDismissLayer:getName()
    return "AppleDismissLayer" --csb = AppleDismiss
end

function AppleDismissLayer:onInit()
    self.super:onInit(self)
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_APPLYDISMISS_WINDOW, handler(self, self.updateInfo))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_APPLYDISMISS_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DOWNLOAD_APPLEDISMISS_WINDOW, handler(self,self.onHeadDoSuccess))
...
function AppleDismissLayer:onShow(data)
    ...
    if self.mIsShow == false then
        ...
        for i=1,3 do
            ...
            self:GetWidgetByName("Text_PlayerNickname",image_PokerResult):setString(laixiaddz.helper.StringRules_6(player[index].Nickname))
...
--                self:GetWidgetByName("Text_IsAgree",image_PokerResult):setString("未应答")
                if self.dataInfo[i].playerId == laixiaddz.LocalPlayercfg.LaixiaPlayerID then --如果这个人是我
                    self:GetWidgetByName("Button_Agree"):setVisible(true)
                    self:GetWidgetByName("Button_Rejust"):setVisible(true)

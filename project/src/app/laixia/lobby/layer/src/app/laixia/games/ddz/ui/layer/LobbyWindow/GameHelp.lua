...
function GameHelp:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_HELP_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_HELP_WINDOW, handler(self, self.destroy))
end
...
function GameHelp:onShow()
    ...
    self.levelArray =  laixiaddz.JsonTxtData:queryTable("gradeArray").buf
    self.gameInstruction = laixiaddz.JsonTxtData:queryTable("gameInstruction").buf

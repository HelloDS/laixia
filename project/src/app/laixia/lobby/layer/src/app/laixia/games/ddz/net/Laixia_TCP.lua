...
function Net_TCP:readyForReConnect()
    ...
    laixiaddz.LocalPlayercfg.reConnect = true --重连他们希望能够直接在X分钟内回到之前的“主界面中”
-- 开始重连
end
...
function Net_TCP:send2Socket(data)
    if not self._socket or not self.isConntected then
        ...
        -- laixiaddz.helper.popupReLoginWindow(laixiaddz.utilscfg.DICT(_ID_DICT_TYPE_NET_ERROR_LOGIN))
        return
    end
    ...
    if mid =="01" and  sid == "05" then
        ...
        laixiaddz.logPacketID("发送消息 "..mid .."000"..sid)
    end
...
function Net_TCP:heart_beat_tick(dt)
    ...
    if laixiaddz.net.mAwakeFlag then
        return
        -- 睡眠不发送心跳
    end
    -- 空闲太久，就心跳一下
    if (self.freeTime >= laixiaddz.config.laixiaddz_HEARTBEAT_INTERVAL) then
        self.freeTime = 0
        laixiaddz.net.sendHeartBeatPacket()
    end
end
...
function Net_TCP:tcpConnect(appID)
    ...
    laixiaddz.LocalPlayercfg.mCurGameAppID = appID or laixiaddz.config.GameAppID;
    local address = laixiaddz.LocalPlayercfg.laixiaddzTCPSever;
    ...
    if self.mEventList and #self.mEventList > 0 then
        ...
        if self.mEvent.eventType == 1 then
            ...
            elseif (self.mEvent.name == cc.net.SocketTCP.EVENT_CONNECT_FAILURE) then
                ...
                laixiaddz.net.clearAllWaiting()
                if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "SingleCardTableDialog" then
                    ui.SingleCardTableDialog.mIsConnect = false
                else
                    if "fish" ~= laixiaddz.config.smallGame then
                        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDJUMP_WINDOW, { msg = "无法连接到服务器，点击重试", msgType = "http" });
                    --cc.UserDefault:getInstance():setBoolForKey("isauto", false)
                    end
                end
                -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW,"无法链接到服务器nettcp")
...
function Net_TCP:close(...)
    ...
    if self._socket then
        ...
        self.isConntected = false
        laixiaddz.net.clearAllWaiting()
        if (laixiaddz.gameStatus == nil) then
            if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "SingleCardTableDialog" then
                ui.SingleCardTableDialog.mIsConnect = false
            else
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDJUMP_WINDOW, { msg = "无法连接到服务器，点击重试", msgType = "http" });

...
function GameListDetailWindow:quitRegistrationFun()
    ...
    stream:setValue("payment_method", "fee1")
    ...
    local function cb(data)
        if data.dm_error == 0 then
            print("比赛报名退出成功！")
            -- scene:popUpTips("退出报名成功")
            ObjectEventDispatch:pushEvent("update_coin_info")
            ...
            -- print("比赛报名退出失败！")
            local scene = cc.Director:getInstance():getRunningScene()
            scene:popUpTips("比赛报名退出失败！"..data.error_msg)
        end
    end
...
function GameListDetailWindow:sendRegistration(msg)
    ...
    if msg.data then
        ...
        if tonumber(laixiaddz.LocalPlayercfg.LaixiaJoinGame) == 1002 then
            baomingfei = "fee2"
        end
        -- baomingfei = "fee1"
        print("道具baomingfei~！",baomingfei)
        stream:setValue("payment_method", baomingfei)
        -- stream:setValue("payment_method", msg.data.PaymentMethod)
        stream:setPostData("uid", laixiaddz.LocalPlayercfg.LaixiaPlayerID)
        laixiaddz.LocalPlayercfg.LaixiaMatchRoom = msg.data.RoomID
        -- dump(msg)
        -- print("msg.data.RoomID = ",msg.data.RoomID)
        -- print("msg.data.payment_method = ",msg.data.PaymentMethod)
        -- print("uid = ",laixiaddz.LocalPlayercfg.LaixiaPlayerID)
        local function cb(data)
            if data.dm_error == 0 then
                print("比赛报名成功！")
                -- scene:popUpTips("报名成功")
                ObjectEventDispatch:pushEvent("update_coin_info")
                local match = laixiaddz.LocalPlayercfg.LaixiaMatchdata
                if #match == 0 then
                    return
                ...
                -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_MATCHLISTDETAIL_WINDOW) -- 关闭
                ...
                -- dump(data)
                -- scene:popUpTips(data.error_msg)
                -- print("比赛报名失败！")
                local scene = cc.Director:getInstance():getRunningScene()
                scene:popUpTips("比赛报名失败！",data.error_msg)
            end
        end
        laixiaddz.net.sendHttpPacketAndWaiting(stream.key,stream,cb)
    end
...
function GameListDetailWindow:updateLineNum()
    ...
    stream:setValue("match_id", laixiaddz.LocalPlayercfg.LaixiaMatchRoom)
    local function cb(data)
        if data.dm_error == 0 then
            -- local match = laixiaddz.LocalPlayercfg.LaixiaMatchdata
            ...
            -- for i = 1, #red_temp do
            --     if (laixiaddz.LocalPlayercfg.LaixiaMatchRoom == red_temp[i].RoomID) then
            --         laixiaddz.LocalPlayercfg.LaixiaMatchdata[laixiaddz.LocalPlayercfg.LaixiaGameListIndex].rooms[i].CurJoinNum = data.data.all_num
            --     end
            -- end
            -- 更新当前进度

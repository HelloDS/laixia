...
local CObjectUIInit = class("CObjectUIInit_sys",laixiaddz.Loader)
...
function CObjectUIInit:PauseFun()
    ...
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_BROADCAST_WINDOW);
    self:DelSceneFunc();
end
...
function CObjectUIInit:backFromSubGame()
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_HALL_WINDOW)  --物理键退出
end

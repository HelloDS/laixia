...
function TPWeekCard:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_WEEKCARD_WINDOW, handler(self, self.show))
end
...
function TPWeekCard:addItems(id)
     local itemData = laixiaddz.JsonTxtData:queryTable("items")
...
function TPWeekCard:onGoShop(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SEND_ORDERINFO_WINDOW,{id=self.ItemID})
    end
end

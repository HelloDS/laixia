...
function GamePlayerObj:ctor(aiPokers, aiSeat,params)
    ...
    dump(self.mPokerData,"self.mPokerData")
    dump(params,"params")
    dump(aiPokers,"aiPokers")
...
function GamePlayerObj:pokerToLaizi(aiPoker)
    local resuTable = { }
    local ReplaceCardss = clone(laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards)
...
function GamePlayerObj:createPokers(aiPoker,params)
    ...
    dump(resutem,"resutem")
    for index = 1, num do
        self.mPokerList[resutem[index].CardValue] = GameCardObject.new(resutem[index])
        self.mPokerList[resutem[index].CardValue]:setLocalZOrder(1010)
    ...
    dump(self.mSeat,"mSeat")
    dump(params.selfSeat,"params.selfSeat")
    --玩家自己的手牌设置滑动触摸位置
...
function GamePlayerObj:setPokerZorder()
    ...
    for index = 15, 1, -1 do
        for i = 2, #self.mPokerData[self.mPokersDataIndex[index]] do  --自己手牌的位置设置
            ...
            if indexT >= 0 then
                self.mPokerList[indexT]:setLocalZOrder(1000+count)
                count = count + 1
            end
        end
    end
    ...
    local setOutLinePosition = function()
            ...
            for i = 2, #self.mPokerData[self.mPokersDataIndex[index]] do
                if self.mPokerData[self.mPokersDataIndex[index]][i] ~= -1 then
                    if ui.CardTableDialog.bDelInfoDisplay == false  then
                        ...
                        self.mPokerList[self.mPokerData[self.mPokersDataIndex[index]][i]]:setLocalZOrder(1000+count)
                        count = count + 1
                    end
                end
            end
        ...
        if #outPoker > 0 then
            ...
            for show1 =1, #outPoker do -- 输出排序后的牌的位置
                ...
                self.mPokerList[outPoker[show1].CardValue]:setLocalZOrder(1000+count)
                count = count + 1
            end
        end
    end
...
function GamePlayerObj:playDealAni(aiTable)
    ...
    function ani(num)
        if num < count then
            ...
            self.mPokerList[t[num]]:setLocalZOrder(1000+num)
            ...
            self:setTouchEnabled(true)
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DEALANI_LANDLORDTABLE_WINDOW);
...
function GamePlayerObj:showMark()
    if self.mSeat == laixiaddz.LocalPlayercfg.LaixiaLandlordSeat then
        local mark = false
        for i = 1,15 do
            for j = #self.mPokerData[self.mPokersDataIndex[i]],2,-1 do
...
function GamePlayerObj:addBottomPokers(aiPokers,aiSeat,aiBar)
    if aiSeat == self.mParams.selfSeat then
        ...
        laixiaddz.loggame("添加地主底牌改变自己牌位置")
        self:setPokerPosition()
        self:showMark()
...
function GamePlayerObj:playHand(aiPokers,aiSeat,params)
    ...
    if seat == selfSeat()  then
        ...
        laixiaddz.loggame("GamePlayerObj:playHand()自己出牌改变位置 ")
        self:setPokerPosition()
    else
...
function GamePlayerObj:chooseChange(aiX)
    ...
    laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_select_sheet)
end
        ...
        for index = 0,53 do
            if self.mPokerList[index] ~= nil then
                if self.mPokerList[index]:isSelect() == true then
                    isHave = true
                    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_COMPUTECART_LANDLORDTABLE_WINDOW,true)
                    break
                end
            end            
        end
        if isHave == false then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_COMPUTECART_LANDLORDTABLE_WINDOW,false)
        end  
    end
end

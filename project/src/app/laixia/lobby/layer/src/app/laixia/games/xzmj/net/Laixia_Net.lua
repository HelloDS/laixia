...
function Net:handlePacket(packet)
    ...
    if packet:getPacketName() ~= "SCHeartBeat" then
        laixiaddz.loggame("handle packet: " .. packet:getPacketName());
    end
...
function Net:onHttpDataRecvHandler(retCode, rawBytes)
    ...
    if (retCode == 404) then
        ...
        -- "无网络"
        -- 暂时先这样。弹出重连的标记
        laixiaddz.loggame("************************** retCode ===== " .. retCode)
        return;
    end
    if(retCode == 500) then
        print ("What is that");
...
function Net:start()
    ...
    if autoValue then
        ...
        if GamePlatformID== 0 or GamePlatformID  > 5 then
            GamePlatformID= 1
    ...
    if laixia.kconfig.isYingKe == true then
        ...
        elseif device.platform == "ios" then
            state,value = luaoc.callStaticMethod("IKCRLXBridgeManager", "userInfo");
            state2,value2 = luaoc.callStaticMethod("IKCRLXBridgeManager", "getToken");
        end
        newGamePlatformID = 9
        print("value")
        print(value)
    ...
    laixiaddz.loggame('send packet ****************** :CSPacketLogin ');

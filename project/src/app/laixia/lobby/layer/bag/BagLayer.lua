...
function BagLayer:sendRequest()
    ...
    laixia.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
        ...
        if data1.dm_error == 0 then
            ...
            if data1 ~= nil and self.onSCPackItemsPacket then
...
function BagLayer:onSCPackItemsPacket(packet)
    ...
    if packet == nil or #tempArray == 0 then
        local Panel_listvecter = _G.seekNodeByName(self.rootNode,"Panel_listvecter")
        local label1 = cc.Label:createWithSystemFont("背包暂无物品", "Arial", 35)  
        label1:setPosition(cc.p(Panel_listvecter:getContentSize().width/2.0, Panel_listvecter:getContentSize().height/2.0))  
        Panel_listvecter:addChild(label1, 1)  
    end
            ...
            if itemMsg.isDelivery == 1 then
                ...
                elseif itemMsg.ItemType == 3 then
                    ...
                    --在这发请求？  还是在最终确认的时候发？
                        ...
                        --1.走兑换的 红包/京东卡   这个暂时没有接口 所以没调  京东卡ItemType改成了5 下面下了
                        if string.sub(sender.ItemID,1,2) == "15" and itemMsg.ObtainItemIDList ~= nil then
                            ...
                            arg.obj = self
                            local popuplayer = require("lobby.layer.bag.PopUpLayer").new(itemMsg.ObtainItemIDList,arg,itemMsg.ItemType)

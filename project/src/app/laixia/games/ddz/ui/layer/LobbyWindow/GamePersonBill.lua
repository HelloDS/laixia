...
function GamePersonBill:onInit()
	...
	ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_PERSONBILL_WINDOW, handler(self, self.show))
	ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_HIDEPERSONBILL_WINDOW, handler(self, self.destroy))
end
...
function GamePersonBill:goBack(sender, eventType)
	if eventType == ccui.TouchEventType.ended then  
		--隐藏该界面
		ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_HIDEPERSONBILL_WINDOW)
	end
end

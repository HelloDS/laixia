...
function TPPhoneLogin:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_PHONELOGIN_WINDOW, handler(self, self.show))
end
...
function TPPhoneLogin:onLogin(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if  string.len(password)<6 then 
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, "请输入6位位数密码")
            return
        end 
      ...
      --"versionName=1.0.0&channelId=0&mobileInfo=huawei-6p&platform=1&account=00-01-6C-D6-01-F6"
        ...
        laixiaddz.LocalPlayercfg.LaixiaPhoneNum = self.phonenumber:getString()
        laixiaddz.LocalPlayercfg.LaixiaPassword = self.password:getString()
        self:onclose()
    end
end
...
function TPPhoneLogin:forgetPassWord(sender, eventType)
    ...
    if eventType == ccui.TouchEventType.ended then
        laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)
        self:destroy()
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FINDPWD_WINDOW)
    end

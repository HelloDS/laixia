...
function RankingListWindow:onInit()
    ...
    self.tag = false
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_RANK_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_RANK_WINDOW, handler(self, self.updateWindow))
...
function RankingListWindow:onShowPersonal(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_USEINFO_WINDOW,newPlayer)
    end
end
...
function RankingListWindow:updateWindow()
    if self.mIsShow then
        ...
        --self.name = laixiaddz.LocalPlayercfg.
...
function RankingListWindow:updateSelfRank()
    ...
    if self.mSelfRank == 0 or self.mSelfRank>50 then
        ...
        if self.indexs == 2 then
            ...
            self:GetWidgetByName("Text_count"):setString(laixiaddz.LocalPlayercfg.SelfRank)
        else
            self:GetWidgetByName("Image_10"):setVisible(true)
            self:GetWidgetByName("Image_9"):setVisible(true)
            self:GetWidgetByName("Text_money"):setVisible(true)
            self:GetWidgetByName("Text_money"):setString(laixiaddz.LocalPlayercfg.LaixiaPlayerGold)
        end
    ...
    self.mSelfRank = 0
    self:addHead(self.playhead ,laixiaddz.LocalPlayercfg.LaixiaPlayerID ,1 ,laixiaddz.LocalPlayercfg.LaixiaPlayerHeadUse )
    self:GetWidgetByName("Text_name_"):setString(laixiaddz.helper.StringRules_6(laixiaddz.LocalPlayercfg.LaixiaPlayerNickname))

...
function TPPersonalWindow:getName()
	return "UserInfoWindow" -- csb = PokerPersonCenter
end

function TPPersonalWindow:onInit()
    self.super:onInit(self)        
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_USEINFO_WINDOW,handler(self,self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_USEINFO_WINDOW,handler(self,self.destroy))
    -- ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_USERINFO_WINDOW,handler(self,self.updateSignature))
    --ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DOWNLOADPERSONAL_PICTURE_WINDOW, handler(self,self.headImageDSuccess))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DOWNLOAD_SELFROOM_PERSONAL_PICTURE_WINDOW, handler(self, self.onHeadDoSuccess))
end
...
function TPPersonalWindow:sendPersonalPacket(userID)
    ...
    CSPlayerRankInfo:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)
...
function TPPersonalWindow:onShow(mesg)
    if self.mIsShow == false then
        ...
        if ui.CardTableDialog.mSelfRoomID == 50 then
            ...
            self:GetWidgetByName("PLA_Label_Gold"):setString(laixiaddz.LocalPlayercfg.LaixiaSelfBuildIntegral)
            ...
            --金币数
            local numbergold = laixiaddz.helper.numeralRules_2(tonumber(mesg.data["gold"]))
            self:GetWidgetByName("PLA_Label_Gold"):setString(tonumber(mesg.data["gold"]))
        end
    
        self:GetWidgetByName("PLA_Label_Name"):setString(laixiaddz.helper.StringRules_6(mesg.data["Name"]))
        ...
        -- local winNum = mesg.data["WinNum"] or 0
        -- local lostNum = mesg.data["LostNum"] or 0
        ...
        -- 胜率
        local winParam = mesg.data["winParam"] or 0
        local labelShengLv = self:GetWidgetByName("PLA_Label_Win")
        labelShengLv:setString(tostring(tonumber(winParam)*100) .. "%")

        -- 冠军次数
        local winNum = mesg.data["winNum"] or 0
        local guanjuncishu = self:GetWidgetByName("PLA_Label_Round") 
        guanjuncishu:setString(winNum)
        ...
        -- 位置的调整
        local panel = self:GetWidgetByName("Panel_13")
        if panel then
            panel:setContentSize(cc.size(display.width,display.height))
            panel:setPosition(cc.p(0,0))
            panel:addTouchEventListener( handler(self,self.shutdown))
            panel = self:GetWidgetByName("Panel_12")
            panel:setPosition(ui.CardTableDialog.posX1 or display.cx, ui.CardTableDialog.posY1 or display.cy)
        end        
        self.mIsShow = true
    end
end
...
function TPPersonalWindow:onHeadDoSuccess(msg)
    ...
    -- print(laixiaddz.LocalPlayercfg.LaixiaPlayerHeadUse)
    -- print(data.savePath)
...
function TPPersonalWindow:addHead(head,id,headIcon_new)
    -- 默认头像图片路径
    self.rankIcon = {}
    headIcon_new = laixiaddz.LocalPlayercfg.LaixiaPlayerHeadUse; --微信头像要用的

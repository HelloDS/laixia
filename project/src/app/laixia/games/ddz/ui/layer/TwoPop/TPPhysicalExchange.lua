...
function TPPhysicalExchange:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_REDEEMENTITY_WINDOW, handler(self, self.show))
end
...
function TPPhysicalExchange:SendPacketToServer(sender, eventtype)
    if eventtype == ccui.TouchEventType.ended then
       ...
       -- local player_name = self.player_name:getString()
        if self.sttr1 == "" then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, "请输入手机号！")
            return
        end
        if self.sttr2 == "" then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, "请输入收货人姓名！")
            return
        end
        -- local phone_number = self.phone_number:getString()
        -- local player_address = self.player_address:getString()
        if self.sttr3 == "" then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MARKEDWORDS_WINDOW, "请输入收货地址！")
            return
        end

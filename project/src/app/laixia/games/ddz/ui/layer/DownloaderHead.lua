...
function DownloaderHead:onResponsePost(event)
    ...
    if (task.mInGame == 1) then --牌桌
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOAD_PICTURE_WINDOW, event)
    elseif(task.mInGame==2) then --大厅
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOADLOBBY_PICTURE_WINDOW, event)
    elseif(task.mInGame ==3) then --排行榜
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOADRANK_PICTURE_WINDOW, event)
    elseif(task.mInGame ==4) then --个人信息
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOADPERSONAL_PICTURE_WINDOW, event)
    elseif(task.mInGame ==6) then-- 站内个人信息 
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOAD_SELFROOM_PERSONAL_PICTURE_WINDOW, event)
    elseif(task.mInGame ==7) then-- 结算
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOAD_RESULT_SMALL_PICTURE_WINDOW, event) 
    elseif(task.mInGame ==8) then-- 自建桌大结算
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOAD_RESULT_BIG_PICTURE_WINDOW, event)       
    elseif(task.mInGame == 5) then  -- 通用图片下载，小游戏可以用这个
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOADNORMAL_PIC_WINDOW, event);
    elseif(task.mInGame == 9) then --解散
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_DOWNLOAD_APPLEDISMISS_WINDOW, event);
    end

...
function ShopWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_SHOP_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_SHOP_WINDOW, handler(self, self.updateWindow))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_HIDE_SHOP_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SEND_ORDERINFO_WINDOW, handler(self, self.sendOrderIDPacket))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SEND_EXCHANGE_WINDOW, handler(self, self.sendExchangePacket))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_EXCHANGE_WINDOW,handler(self,self.updateExchange))
end

--发送兑换请求
function ShopWindow:sendExchangePacket()
    local ExchangePacket = Packet.new("ExchangePacket", _LAIXIA_PACKET_CS_ExchangeDataID)
    ExchangePacket:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)
...
function updateUserCoin(data)
    ...
    self:GetWidgetByName("Text_6"):setString(laixiaddz.LocalPlayercfg.ZhiShiBiNum)
end
-- function updateUserCoin_IOS(status,data)
--     print("ShopWindow---------------------alexwang")
--     print(data)
--     if status == 0 then
--         laixiaddz.LocalPlayercfg.ZhiShiBiNum = tonumber(data)
--         self:GetWidgetByName("Text_6"):setString(laixiaddz.LocalPlayercfg.ZhiShiBiNum)
--     end
-- end
...
--------------------------------------------------------------------------P---------------------------
        self.mAllItems = {}      
        local goods = laixiaddz.JsonTxtData:queryTable("shop"):queryAllMallInfoByOneList();
        if laixiaddz.kconfig.isYingKe ~= true then   
            for i,v in ipairs(goods) do
                ...
                elseif device.platform == "ios" then
                    if tostring(laixiaddz.LocalPlayercfg.CHANNELID)=="111267" then--官网
                        if tonumber(v.itemsID)<130000 and tonumber(v.itemsID)>=120000 then
                            isUse = true
                        end
                    elseif tostring(laixiaddz.LocalPlayercfg.CHANNELID)=="201009" or tonumber(laixiaddz.LocalPlayercfg.CHANNELID)>=200000 then--苹果渠道包
                        if tonumber(v.itemsID)<130000 and tonumber(v.itemsID)>=120000 then
                            isUse = true
                        end
        ...
        elseif laixiaddz.kconfig.isYingKe == true then
            for i,v in ipairs(goods) do
                local isUse = false
                if device.platform == "android" then
                ...
                elseif device.platform == "ios" then
                    if tostring(laixiaddz.LocalPlayercfg.CHANNELID)=="201010" then
                        if tonumber(v.itemsID)<140000 and tonumber(v.itemsID)>=130000 then
                            isUse = true
                        end
                    elseif tostring(laixiaddz.LocalPlayercfg.CHANNELID)=="111267" then--官网
                        if tonumber(v.itemsID)<140000 and tonumber(v.itemsID)>=130000 then
                            isUse = true
                        end
                    elseif tostring(laixiaddz.LocalPlayercfg.CHANNELID)=="201009" or tonumber(laixiaddz.LocalPlayercfg.CHANNELID)>=200000 then--苹果渠道包
...
----显示公共的头
       ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_COMMONTOP_WINDOW,
        { goBackFun = function()
            if laixiaddz.LocalPlayercfg.OnReturnFunction == _laixiaddz_EVENT_UPDATE_SELECTROOM_WINDOW then
            ...
            local stream = Packet.new("EnterListRoom", _LAIXIA_PACKET_CS_ListRoomID)
                  ...
                  ObjectEventDispatch:pushEvent(laixiaddz.LocalPlayercfg.OnReturnFunction)
                  
            end
            --ObjectLocalPlayerData.OnReturnFunction = _laixiaddz_EVENT_SHOW_HALL_WINDOW
          end,
...
function ShopWindow:onShowExchageDetail(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)
        ObjectEventDispatch:dispatchEvent( { name = _laixiaddz_EVENT_SHOW_REDEEMDETAILS_WINDOW, Info = sender.Info })
    end
end
...
function ShopWindow:adGoods2Listview(start,over,mode)
    if laixiaddz.kconfig.isYingKe ~= true then  
        for i=start,over do 
            ...
            local itemInfo = laixiaddz.JsonTxtData:queryTable("items"):query("ItemID",itemID)
           -- local itemInfo2 = laixiaddz.JsonTxtData:queryTable("shop"):query("itemsID",itemID);
...
function ShopWindow:sendOrderIDPacket(msg)
    if msg and msg.data and msg.data.id then
        ...
        CSRequestOrder:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)
...
function ShopWindow:buy(sender, eventType)
    ...
    --         ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_WEEKCARD_WINDOW,{id=sender.ID}) 
    --     else
    --         self:sendOrderIDPacket({data={id=sender.ID}})
    --     end   
    -- end
    ...
    print("55555555555555555555555555555555555555555555555")
        if laixiaddz.kconfig.isYingKe == true then
            if sender.ZhiShiBiNum > (laixiaddz.LocalPlayercfg.ZhiShiBiNum or 0) then
                if device.platform == "android" then
                    local javaClassName = APP_ACTIVITY
...
--         CSRequestExchangeJB:setValue("Code", laixiaddz.LocalPlayercfg.LaixiaHttpCode)

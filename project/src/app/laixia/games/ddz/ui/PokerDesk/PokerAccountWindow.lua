...
function PokerAccountWindow:getName()
    return "PokerAccountWindow" -- csb = PokerResultWindow
end
...
function PokerAccountWindow:onShow(data)
   ...
   -- self.mTime = self:GetWidgetByName("Label_Time_Self")
    ...
    --
    if laixiaddz.LocalPlayercfg.LaixiaSelfBuildTable ~= nil and laixiaddz.LocalPlayercfg.LaixiaSelfBuildTable > 0 or (self.info and tonumber(self.info.gametype) == 4 )then
        ...
        if roomInfo and roomInfo["fuwufei"] then
        ...
        cc.CallFunc:create(
            function()
                ...
                if self.isNeedShowLaidou~=nil then
                    self:GetLaidou(self.isNeedShowLaidou)
                end
            end),
...
function PokerAccountWindow:GetLaidou(msg)
    if self.mIsShow and msg then
        self.isNeedShowLaidou = msg
        local data = self.isNeedShowLaidou.data
        if data and data.laidou and data.item_id then
            if tonumber(data.laidou) > 0 and tonumber(data.item_id) == 1002 then
                self.ld:setVisible(true)
                self.ldNum:setVisible(true)
                self.ldNum:setString(data.laidou)
...
function PokerAccountWindow:goBackFun(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ui.CardTableDialog.isPlay = false
        laixiaddz.LocalPlayercfg.LaixiaisTrusteeship = false
        laixiaddz.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)

        dump(laixiaddz.LocalPlayercfg.LaixiaSelfTotalInning,"laixiaddz.LocalPlayercfg.LaixiaSelfTotalInning")
        ...
        dump(_laixiaddz_EVENT_EXITROOM_LANDLORDTABLE_WINDOW,"_laixiaddz_EVENT_EXITROOM_LANDLORDTABLE_WINDOW")

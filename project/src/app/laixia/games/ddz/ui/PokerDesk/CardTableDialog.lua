...
local l_PacketObj = laixiaddz.Packet                                             
...
require "games.ddz.ui.PokerDesk.ddzLgcObject"
require "games.ddz.ui.PokerDesk.ddzAISelCard"
...
function CardTableDialog:getName()
    return "CardTableDialog"; --csb = PokerDeskWindow
end  
...
function CardTableDialog:onInit()
    ...
    -- 注册事件
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CLEAR_LANDLORDTABLE_WINDOW, handler(self, self.GameClearCardFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_TALKINGINFO_WINDOW, handler(self, self.ShowTalkingFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CLEAR_DATELANDLORDTABLE_WINDOW, handler(self, self.clearData))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SPRING_LANDLORDTABLE_WINDOW, handler(self, self.SpringAnimationFunction)) --结算
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CONTINUEGOLD_LANDLORDTABLE_WINDOW,handler(self,self.OnChangeGoldContinue))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_COMPUTECART_LANDLORDTABLE_WINDOW,handler(self,self.GameSetPlayStyleFunction));

    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_BEGIN_LANDLORDTABLE_WINDOW,handler(self,self.OnbeginFunction)) 
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_NOMONEY_LANDLORDTABLE_WINDOW,handler(self,self.nomoneyFunction)) --修改用户没钱已离开房间的状态
 
        
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_LANDLORDTABLE_WINDOW, handler(self, self.show))

    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_INITSYN_LANDLORDTABLE_WINDOW, handler(self, self.OnInitSynchronousFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DEAL_LANDLORDTABLE_WINDOW, handler(self, self.OnSendDealFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CALL_LANDLORDTABLE_WINDOW, handler(self, self.GamethebeFunction))   --叫分
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_PLAY_LANDLORDTABLE_WINDOW, handler(self, self.GamePlayFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOWCARD_LANDLORDTABLE_WINDOW, handler(self, self.showCardFunction)) --明牌消息
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_MANDATE_LANDLORDTABLE_WINDOW, handler(self, self.cancelHostingFunction))  --托管
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_SYNTABLESTAGE_WINDOW, handler(self, self.synchronousStepDataFunction))   --牌桌阶段同步
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_ATTRICHANGE_LANDLORDTABLE_WINDOWS, handler(self, self.attributeChangeFunction))

    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DEALANI_LANDLORDTABLE_WINDOW, handler(self, self.GameDealAniFunction))   --播放发牌动画


    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_CONTINUE_LANDLORDTABLE_WINDOW,handler(self,self.OnSendContinueGameFunction)) --发送继续打牌
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_EXITROOM_LANDLORDTABLE_WINDOW,handler(self,self.GameSendLeaveRoomFunction))   --发送离开房间消息
    
    --添加流局动画
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_ANIMATION_LIUJU, handler(self, self.onCreatLiuJuAniFunction))
    --初始化自建桌的ui显示   
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_SELFBUILDINGUI, handler(self, self.OnRefreshSelfBuildUIFunction))
    --继续游戏显示界面消息  
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATE_CONTINUESELFBUILD, handler(self, self.OnConSelfBuildFunction))
    --自建房有人退出界面显示
    ObjectEventDispatch:addEventListener(_LAIXIA_ECENT_UPDATE_UIINEXIT, handler(self, self.OnRefreshUIinExitFunction))

    --更新金、奖券数量
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATAGOLD, handler(self, self.GameRefreshGoldFunction))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_WAITTINGCARTOON_WINDOW,handler(self,self.OnWaitingAniFunction))
 
    -- ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DOWNLOAD_PICTURE_WINDOW, handler(self, self.OnNewHeadXiazaiFunction))
    --自建房大结算显示金币变化
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_UPDATEGOLD, handler(self, self.OnRefreshUIGold))

    --ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_FAILE_WINDOW,handler(self, self.FaileAnimate))
    --监听解散状态
--    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW,handler(self,self.showAppleDissmissLayer))
    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_DOWNLOAD_PICTURE_WINDOW, handler(self, self.onHeadDoSuccess))

    ObjectEventDispatch:addEventListener(_laixiaddz_EVENT_SHOW_HALL_WINDOW, handler(self, self.backHall))

    ObjectEventDispatch:addEventListener("tableUpdateRankInfo", handler(self, self.updateRankInfo))
...
function CardTableDialog:OnChangeGoldContinue(msg)
    ...
    self.mGold:setString(laixiaddz.helper.numeralRules_2(msg.data.Gold)) 
    ...
    if self.mSynchronousData ~= nil then
        for k,v in pairs(self.mSynchronousData.Players) do
            if v.PID == laixiaddz.LocalPlayercfg.LaixiaPlayerID  and( v.CardCountTime == -1 or v.CardCountTime == 0 ) then
                flag = true
            end
        end
    end
    ...
    -- laixiaddz.soundTools.playMusic(laixiaddz.soundcfg.SCENE_MUSIC.nomal,true);
end
...
function CardTableDialog:onTick(dt)
    if self.isShow then
        ...
        laixiaddz.LocalPlayercfg.LaixiaSelfBuildIntegral = self.mGold:getString()
...
--        if self.mPokerDeskType == 4 then
--            self:GetWidgetByName("Label_Current_Ranking"):setString(laixiaddz.LocalPlayercfg.LaixiaMatchRank.."/"..laixiaddz.LocalPlayercfg.LaixiaMatchTotalNum) --隐藏比赛名次
--        end
                ...
                if self.mStageStep ~= nil and self.mSynchronousData ~= nil then
                    ...
                    if temp >= 0 then
                        ...
                        if temp ~= showTime and ( temp <= 5 and temp > 0 and temp ~= 3 ) and self.isPlay then
                            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_event_table_count_down)
                        end
                    else
        ...
        if self:GetWidgetByName("Label_Current_Integral",self.mRankingBG) and self.mPokerDeskType == 4 then
            if laixiaddz.LocalPlayercfg.LaixiaMatchStage == 1 then 
                self:GetWidgetByName("Label_Current_Integral",self.mRankingBG):setString("当前排名："..laixiaddz.LocalPlayercfg.LaixiaMatchRank.."/"..laixiaddz.LocalPlayercfg.LaixiaMatchTotalNum)
            elseif laixiaddz.LocalPlayercfg.LaixiaMatchStage == 3 then 
                if tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString())) == 0 and tonumber(laixiaddz.helper.ConvertStrToNum(self.mLeftInfoGold:getString())) == 0
                    and tonumber(laixiaddz.helper.ConvertStrToNum(self.mRightInfoGold:getString())) == 0 then
                    self:GetWidgetByName("Label_Current_Integral",self.mRankingBG):setString("当前排名：1/3")
                elseif tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString())) > tonumber(laixiaddz.helper.ConvertStrToNum(self.mLeftInfoGold:getString())) 
                    and tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString())) > tonumber(laixiaddz.helper.ConvertStrToNum(self.mRightInfoGold:getString())) then
                    self:GetWidgetByName("Label_Current_Integral",self.mRankingBG):setString("当前排名：1/3")
                    laixiaddz.LocalPlayercfg.NowRankinSNG = 1
                elseif tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString())) < tonumber(laixiaddz.helper.ConvertStrToNum(self.mLeftInfoGold:getString())) and tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString()))< tonumber(laixiaddz.helper.ConvertStrToNum(self.mRightInfoGold:getString())) then
                    self:GetWidgetByName("Label_Current_Integral",self.mRankingBG):setString("当前排名：3/3")
                    laixiaddz.LocalPlayercfg.NowRankinSNG = 3
                else
                    self:GetWidgetByName("Label_Current_Integral",self.mRankingBG):setString("当前排名：2/3")
                    laixiaddz.LocalPlayercfg.NowRankinSNG = 2
                end
            end 
        end   
    end
...
function CardTableDialog:OnRefreshIconOfBag()
    if self.isShow == true then 
        laixiaddz.UItools.udpateSuperGiftIcon(self.Button_supergift)
    end
end
...
function CardTableDialog:InitSelfBuildFunc()
  ...
  if self.mSelfRoomID ~= 50  then
       ...
       dump(self.mSelfRoomType,"=========self.mSelfRoomType")
       dump( self.mPokerDeskType,"==========self.mPokerDeskType")
       return 
   else
     --托管置灰
  ...
  --  self:GetWidgetByName("Label_Current_Coin",self.selfjifen):setString("0")
   ...
   -- self:GetWidgetByName("Label_RightInfoGold",self.rightinfo):setString("0")
      ...
      for i,v in ipairs(headBG) do  --显示房主图标和添加好友按钮
        ...
        if self.mMasterID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then --自建房自己是房主
        ...
        -- if self.mMasterID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
        ...
        if self.hDeskID  ~= nil then
            laixiaddz.LocalPlayercfg.LaixiaSelfBuildTable  =   self.hDeskID
...
function CardTableDialog:onShow(msg) 
    ...
    -- laixiaddz.soundTools.playMusic(laixiaddz.soundcfg.SCENE_MUSIC.nomal,true);
    laixiaddz.soundTools.stopMusic()
    
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_TABLERESULT_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_MATCHWAITLOADING_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_DAILYSIGN_WINDOW)
    --ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MATCHFLICKERWORDS_WINDOW)

    laixiaddz.LocalPlayercfg.laixiaddzisConnectCardTable = true
    ...
    if not self.isShow then
        ...
        l_RoomDataMgr =  laixiaddz.JsonTxtData:queryTable("room_list");
        
        self.hTouxiangImageWidge =  self:GetWidgetByName("Image_Head")
        self:OnRefreshHeadIconFunction(self.hTouxiangImageWidge,laixiaddz.LocalPlayercfg.LaixiaPlayerID,laixiaddz.LocalPlayercfg.HEAD_URL)
        ...
        -- end
        dump(self.mPokerDeskType,"self.mPokerDeskType")
        dump(self.mSelfRoomID,"self.mSelfRoomID")
        ...
        self:OnRefreshHeadIconFunction(self.mSelfHead,laixiaddz.LocalPlayercfg.LaixiaPlayerID,laixiaddz.LocalPlayercfg.HEAD_URL)
        ...
        -- self.Label_selfInfoName:setString(laixiaddz.LocalPlayercfg.LaixiaPlayerNickname)
...
-----------------------------------------------------------------------------------
        ...
        self.matchInsId = msg.data.MatchInsId
...
-----------------------------------------------------------------------------------------
    ...
    if msg.data.isAppleDissmis == 7 then
        if ui.AppleDismissLayer.mIsShow == false then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW,{data = msg.data})
        elseif ui.AppleDismissLayer.mIsShow == true then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_UPDATE_APPLYDISMISS_WINDOW,{data = msg.data})
        end 
--        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW,{data=msg.data})
    end
    ...
    if self.mSelfRoomID ~= 50 then
         self:GaibianShuziWenbenFunction(self.mGold,laixiaddz.LocalPlayercfg.MatchGold or laixiaddz.LocalPlayercfg.LaixiaGoldCoin )
    end
  
    if self.mSelfRoomType == 4 then
    ...
    if self.mPokerDeskType == 1 or self.mPokerDeskType == 2 or self.mPokerDeskType == 0 then
        if not self.intoWindowsData.EnterType or self.intoWindowsData.EnterType == 0 then
            if laixiaddz.LocalPlayercfg.LaixiaMatchRoomType == 0 then--定时比赛
    ...
    if self.mPokerDeskType == 4  then    --比赛场
        ...
        local system = laixiaddz.ani.CocosAnimManager   --比赛开始动画
        self.matchGamestartAni = system:playAnimationAt(self.hPanelImageWidge,"doudizhu_matchstart")
        self.matchGamestartAni:setLocalZOrder(1005)
        ...
        if laixiaddz.LocalPlayercfg.LaixiaMatchRoomType == 0 then --mtt播放第一阶段的动画
            self.root:runAction(cc.Sequence:create(
                ...
                cc.CallFunc:create(
                    function()   
                        ...
                        self.firststageAni:setLocalZOrder(1005)
...
function  CardTableDialog:OnRequestPengyou(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        local Url = "http://wx.laixiaddz.com/download"
...
function CardTableDialog:OnSelfSyncFunc(msg)
    ...
    if msg.TotalInning == 0 then
      ...
      laixiaddz.LocalPlayercfg.LaixiaSelfTotalInning = msg.TotalInning
      laixiaddz.LocalPlayercfg.LaixiaSelfInning =  msg.Inning

      local Inning =  laixiaddz.LocalPlayercfg.LaixiaSelfInning 
...
function CardTableDialog:OnSendBuildDeleteFunction(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        -- if self.mMasterID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
            ...
            laixiaddz.net.sendPacket(onSendDelTable)
        -- else
        --   sender:setVisible(false)
        --   ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"只有房主才能解散房间") 
        -- end
...
function CardTableDialog:OnRefreshSelfBuildUIFunction(msg)
    ...
    for i,v in ipairs(msg.data.Players) do
        ...
        elseif mySeat == ( v.Seat + 4 )%3  then
            self.mLeftInfoName:setString(laixiaddz.helper.StringRules_6(v.Nickname))
        ...
        elseif mySeat == ( v.Seat + 2 )%3  then
            self.mRightInfoName:setString(laixiaddz.helper.StringRules_6(v.Nickname))
...
function CardTableDialog:addHeadIcon(head_btn,path)
    ...
    laixiaddz.UItools.addHead(head_btn, path, templet)
...
--         -- -- 再添加新头像
--         local templet = soundConfig.IMG_HEAD_TEMPLET_RECT
--         laixiaddz.UItools.addHead(head, path, templet)
...
function CardTableDialog:OnWaitingAniFunction()
    if self.isShow then
        ...
        if self.mSelfRoomID ~= 50 and self.mPokerDeskType ~= 4 then
            ...
            self.matchingAni:setLocalZOrder(1005)
...
function  CardTableDialog:OnWaitingAniFunctionForDingshiMatch()
    if self.isShow then
        ...
        if self.mSelfRoomID ~= 50 and self.mPokerDeskType ~= 4 then
            ...
            self.laterAni:setLocalZOrder(1005)
...
--    end
    ...
    self.mRoomID = 0
    laixiaddz.LocalPlayercfg.LaixiaRoomID = 0
    laixiaddz.LocalPlayercfg.LaixiaisConnectCardTable = false 
    self:clearTexture()
    laixiaddz.LocalPlayercfg.LaixiaSelfBuildTable = -1 --初始化自建房ID
    laixiaddz.LocalPlayercfg.LaixiaSelfTotalInning = 0 --初始化自建房次数
    laixiaddz.LocalPlayercfg.LaixiaSelfInning =  0
    self:stopTimer()
end 
...
function CardTableDialog:clearData(msg)
    ...
    self.mMySeatID = -1
    laixiaddz.LocalPlayercfg.LaixiaMySeat = -1
    laixiaddz.LocalPlayercfg.LaixiaLandlordSeat = -1

    laixiaddz.LocalPlayercfg.LaixiaOutCardsCount = 0
    ...
    laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards={} -- 清理牌桌的时候一定要吧癞子匹配的牌清理了，否则会出现数值污染
    self.hLaiziCard =-1
    ...
    -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_CHANGE_BUYRECORDER_WINDOW,false); 
...
function CardTableDialog:OnInitSynchronousFunction(msg)
    ...
    laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards={} 
    self.mDiscardCards = nil
    if self.isShow ~= true then
        local tCardCountTime
        for i = 1,3 do
            if msg.data.Players[i].PID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
    ...
    for k,v in ipairs(msg.data.Players) do
        ...
        elseif laixiaddz.LocalPlayercfg.LaixiaMySeat == ( v.Seat + 4 )%3  then
            self.mLeftInfoName:setString(laixiaddz.helper.StringRules_6(v.Nickname))
            self:GaibianShuziWenbenFunction(self.mLeftInfoGold,v.Coin)
            self.mLeftHead.Sex=v.Sex
        ...
        elseif laixiaddz.LocalPlayercfg.LaixiaMySeat == ( v.Seat + 2 )%3  then
            self.mRightInfoName:setString(laixiaddz.helper.StringRules_6(v.Nickname))
            self:GaibianShuziWenbenFunction(self.mRightInfoGold,v.Coin)
            self.mRightHead.Sex=v.Sex 
            local cards  = v.PlayCards
    ...
    if msg.data.Stage == 3 then
        ...
        laixiaddz.LocalPlayercfg.LaixiaOutCards = {}                       
        laixiaddz.LocalPlayercfg.LaixiaOutCards.Cards = msg.data.Players[topSeatIndex].PlayCards--msg.data.Players[topSeatIndex].DeskCards
        if #laixiaddz.LocalPlayercfg.LaixiaOutCards.Cards == 0 then
           laixiaddz.LocalPlayercfg.LaixiaOutCards.Cards = msg.data.Players[nextSeatIndex].PlayCards
        end


        laixiaddz.LocalPlayercfg.LaixiaOutCards.Seat = msg.data.Players[topSeatIndex].Seat
        laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex = {}

        dump(laixiaddz.LocalPlayercfg.LaixiaOutCards.Seat,"laixiaddz.LocalPlayercfg.LaixiaOutCards.Seat")
        if #msg.data.Players[topSeatIndex].DeskCards == 0 then
            count  = 1
        else
            isHavePlay = true
            laixiaddz.LocalPlayercfg.LaixiaOutCardsCount = 1 
            laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex = laixiaddz.LocalPlayercfg.LaixiaOutCards.Cards
        end
        ...
        laixiaddz.LocalPlayercfg.LaixiaOutCards.count = count
    end
        ...
        self.mBottomPoker[7]:setScale(0.2)
         ...
         for i = 1,3 do
            if self.mBottomPoker[3+i] == 0 then
                ...
                self.mBottomPoker[i]:setLocalZOrder(1000+i*10)    
            else
                self.mBottomPoker[3+i]:setPositionX(637+(i-2)*56)--630-56+(i-1)*28)
                self.mBottomPoker[3+i]:setLocalZOrder(1000+i*10)              
            end
    ...
    if self.mPokerDeskType == 4 then
        ...
        -----------
        -----------
        if laixiaddz.LocalPlayercfg.LaixiaMatchStage == 1 or laixiaddz.LocalPlayercfg.LaixiaMatchStage == 2 then
            if laixiaddz.LocalPlayercfg.LaixiaMatchStage == 1 then
                if tonumber(laixiaddz.helper.ConvertStrToNum(self.mGold:getString())) <tonumber(laixiaddz.helper.ConvertStrToNum(self.mResultNum:getString())) * 6 then
                    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "如果积分低于0，您将被淘汰出局")
                end
            end
            ...
            if laixiaddz.LocalPlayercfg.LaixiaMatchStage == 3 then
                if laixiaddz.LocalPlayercfg.LaixiaMatchlastStage ~= laixiaddz.LocalPlayercfg.LaixiaMatchStage and laixiaddz.LocalPlayercfg.mOfflineMatch == false then
                    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_MATCHFLICKERWORDS_WINDOW, "决赛开始：定局积分")
                end
            end
        ...
        if laixiaddz.LocalPlayercfg.LaixiaMatchName == "" then
            if laixiaddz.LocalPlayercfg.LaixiaMatchlastStage ~= laixiaddz.LocalPlayercfg.LaixiaMatchStage then
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "")
                laixiaddz.LocalPlayercfg.LaixiaMatchlastStage = laixiaddz.LocalPlayercfg.LaixiaMatchStage
            end

            if laixiaddz.LocalPlayercfg.LaixiaMatchDifen < tonumber(self.mResultNum:getString()) then
                laixiaddz.LocalPlayercfg.LaixiaMatchDifen=tonumber(self.mResultNum:getString())
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, "底分已增长至" .. self.mResultNum:getString())
            end
        end
        if laixiaddz.LocalPlayercfg.LaixiaMatchName ~= "" then
            if laixiaddz.LocalPlayercfg.mOfflineMatch == true then
                laixiaddz.LocalPlayercfg.mOfflineMatch = false
            elseif laixiaddz.LocalPlayercfg.LaixiaMatchRoomType == 1 then
--                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, laixiaddz.LocalPlayercfg.LaixiaMatchName .. "开始")   -- 进入什么比赛飘字end
            end
            cc.UserDefault:getInstance():setStringForKey("MatchName", laixiaddz.LocalPlayercfg.LaixiaMatchName )
            --laixiaddz.LocalPlayercfg.LaixiaMatchName = ""
        end
        laixiaddz.LocalPlayercfg.LaixiaMatchDifen=tonumber(self.mResultNum:getString())
        laixiaddz.LocalPlayercfg.LaixiaMatchlastStage = laixiaddz.LocalPlayercfg.LaixiaMatchStage
    else
...
function CardTableDialog:GaibianShuziWenbenFunction(aiLable,aiGold)
    ...
    -- end
    aiLable:setString(laixiaddz.helper.numeralRules_2(aiGold))
end
...
function CardTableDialog:ShowResultMatchFunction(mesg) 
    ...
    for i, v in ipairs(data) do
        ...
        if self.myPID == v.PID then
            ...
            --self.mGold:setString(laixiaddz.helper.numeralRules_2(v.CurrentGold))
            if v.Chip > 0 then
                str = v.Chip
...
--    aiData.data.jixu = self.jixuyouxi
    ...
    if type(aiData.data) == "table" then
        ...
        for i = 1,3 do
            if laixiaddz.LocalPlayercfg.LaixiaPlayerID == self.mDeskResult.CSBalance[i].PID and self.mDeskResult.CSBalance[i].Chip >= 0  then
                self.isWinByDeskResult = true
            end
        end

        dump(self.mDeskResult,"self.mDeskResult")
        ...
        if self.isWinByDeskResult then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_event_table_win);
        else
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_event_table_lost);
        end
        --显示钱变动和剩余牌的情况
        if (self.mPokerDeskType == 1 or self.mPokerDeskType == 2 or self.mPokerDeskType == 0) then
    ...
    local onShowAnimationfun = cc.CallFunc:create(function ()
        ...
        if self.mDeskResult.CSBalance[1].Chip==0 and self.mDeskResult.CSBalance[2].Chip==0 and self.mDeskResult.CSBalance[3].Chip==0 then
            ...
            mAnimation:addTo(self.hPanelImageWidge,1005) 
...
function CardTableDialog:talkInfoFunction(aiType,aiInfo,aiSeat)  --aiSeat已经转换，已经是相对于自己的位置
    ...
    elseif 0 == aiType then
        ...
        if self.mMySeatID == aiSeat then
            ...
            imageface:setLocalZOrder(10100)
...
function CardTableDialog:playSound(aiSex,aiType,aiPokerIndex)
        ...
        if aiType == l_CardTypeObj.SINGLE_CARD then
            if aiPokerIndex == 0 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_0"])
            elseif aiPokerIndex == 1 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_1"])
            elseif aiPokerIndex == 2 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_2"])
            elseif aiPokerIndex == 3 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_3"])
            elseif aiPokerIndex == 4 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_4"])
            elseif aiPokerIndex == 5 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_5"])
            elseif aiPokerIndex == 6 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_6"])
            elseif aiPokerIndex == 7 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_7"])
            elseif aiPokerIndex == 8 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_8"])
            elseif aiPokerIndex == 9 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_9"])
            elseif aiPokerIndex == 10 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_10"])
            elseif aiPokerIndex == 11 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_11"])
            elseif aiPokerIndex == 12 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_12"])
            elseif aiPokerIndex == 13 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_13"])
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_14"])
            end
        elseif aiType == l_CardTypeObj.DOUBLE_CARD then
            if aiPokerIndex == 0 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_0"])
            elseif aiPokerIndex == 1 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_1"])
            elseif aiPokerIndex == 2 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_2"])
            elseif aiPokerIndex == 3 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_3"])
            elseif aiPokerIndex == 4 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_4"])
            elseif aiPokerIndex == 5 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_5"])
            elseif aiPokerIndex == 6 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_6"])
            elseif aiPokerIndex == 7 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_7"])
            elseif aiPokerIndex == 8 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_8"])
            elseif aiPokerIndex == 9 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_9"])
            elseif aiPokerIndex == 10 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_10"])
            elseif aiPokerIndex == 11 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_11"])
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_two_pieces_12"])
            end
        elseif aiType == l_CardTypeObj.THREE_CARD then
            if aiPokerIndex == 0 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_0"])
            elseif aiPokerIndex == 1 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_1"])
            elseif aiPokerIndex == 2 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_2"])
            elseif aiPokerIndex == 3 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_3"])
            elseif aiPokerIndex == 4 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_4"])
            elseif aiPokerIndex == 5 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_5"])
            elseif aiPokerIndex == 6 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_6"])
            elseif aiPokerIndex == 7 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_7"])
            elseif aiPokerIndex == 8 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_8"])
            elseif aiPokerIndex == 9 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_9"])
            elseif aiPokerIndex == 10 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_10"])
            elseif aiPokerIndex == 11 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_11"])
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_triplet_12"])
            end
        elseif aiType == l_CardTypeObj.THREE_ONE_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_3_with_1"])
        elseif aiType == l_CardTypeObj.THREE_TWO_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_3_with_2"])
        elseif aiType == l_CardTypeObj.BOMB_CARD or aiType == l_CardTypeObj.SOFT_BOMB_CARD or aiType == l_CardTypeObj.LAIZI_BOMB_CARD or aiType == l_CardTypeObj.ROCKET_CARD then
            if aiPokerIndex == 13 or aiPokerIndex == 14 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_king_bomb"])
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_bomb"])
            end
        elseif aiType == l_CardTypeObj.BOMB_TWO_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_4_with_2"])
        elseif aiType == l_CardTypeObj.CONNECT_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_straight"])
        elseif aiType == l_CardTypeObj.COMPANY_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_even_straight"])
        elseif aiType == l_CardTypeObj.AIRCRAFT_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_airplane"])
        elseif aiType == l_CardTypeObj.AIRCRAFT_SINGLE_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_airplaneDaichibang"])
        elseif aiType == l_CardTypeObj.AIRCRAFT_DOBULE_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_airplaneDaichibang"])
        elseif aiType == l_CardTypeObj.BOMB_TWOOO_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_4_with_2_2"])
        else
            --错误牌型
        end
end
...
function CardTableDialog:playPassSound(aiSex)
       ...
       if aiSex == 0 then
        ...
        if   typeIndex == 1 then 
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_pass_1"])
        elseif  typeIndex == 2 then 
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_pass_2"])
        else
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_pass_3"])
        end
...
function CardTableDialog:showHuoJianAni()
    ...
    self.rocket:setLocalZOrder(1004)
    ...
    cc.CallFunc:create(function() 
            if device.platform == "android" and laixiaddz.LocalPlayercfg.LaixiaIsShake == 1 then
                local luaj = require "cocos.cocos2d.luaj"
                local params = { 700 }
...
function CardTableDialog:CardEffectFunction(aiType)
    ...
    local outPokersData = laixiaddz.LocalPlayercfg.LaixiaOutCards
    ...
    if #outPokersData.Cards > 0 then
        self.mNewOutType = laixiaddz.LocalPlayercfg.LaixiaOutCardsCount
       if  aiType == l_CardTypeObj.ROCKET_CARD and #outPokersData.Cards > 0 then
            self:HideAnimationFunction()
            -- laixiaddz.soundTools.playMusic(laixiaddz.soundcfg.SCENE_MUSIC.boom,true);
        ...
        elseif aiType ==l_CardTypeObj.BOMB_CARD or aiType == l_CardTypeObj.BOMB_CARD or aiType == l_CardTypeObj.SOFT_BOMB_CARD or aiType == l_CardTypeObj.LAIZI_BOMB_CARD  and #outPokersData.Cards > 0 then 
            self:HideAnimationFunction()
            local seat = outPokersData.Seat
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_bomb) 
            -- laixiaddz.soundTools.playMusic(laixiaddz.soundcfg.SCENE_MUSIC.boom,true);
            if device.platform == "android" and laixiaddz.LocalPlayercfg.LaixiaIsShake == 1 then
                local params = { 700 }
                local state, value = luaj.callStaticMethod(JAVA_SHAKE_CLASSPATH, "gameShake", params, "(I)V")
        ...
        elseif  aiType == l_CardTypeObj.AIRCRAFT_SINGLE_CARD or aiType == l_CardTypeObj.AIRCRAFT_DOBULE_CARD or aiType == l_CardTypeObj.AIRCRAFT_CARD   then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_airplane)
            self:HideAnimationFunction()    

            -- ###########
            local system = laixiaddz.ani.CocosAnimManager
            local node = system:playAnimationAt(self.Panel_1,"doudizhu_plane")
            node:setLocalZOrder(1100)
        ...
        elseif aiType == l_CardTypeObj.COMPANY_CARD then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_straight)
            self:HideAnimationFunction()   
...
function CardTableDialog:CreateClockFunction()
    ...
    self.mClock:setLocalZOrder(1005)
...
--   end
    ...
    self.dzanimmal:setLocalZOrder(1005)
...
function CardTableDialog:OnSendDealFunction(msg)
    ...
    if #tDeal.Cards == 3 then
            ...
            transition.execute(aiActor, cc.ScaleTo:create(0.3,0,0.33,1), {
                ...
                onComplete = function()
                    ...
                    self.mBottomPoker[index+3] = l_cardobj.new(temptable)
                              :pos(x,y)
                              :addTo(parent)
                    self.mBottomPoker[index+3]:setLocalZOrder(1000+index*10) 
        ...
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_deal_card)
        ...
        if tDeal.Reelect == 1 then
            ...
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_ANIMATION_LIUJU)          
        end
    end
end
...
function CardTableDialog:dealHandPokersFunction(aiDeal)
    ...
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_MATCHRANK_WINDOW) --隐藏中间的等级界面
    self:DisplayWhichClockFunction(0)
    local tDeal = aiDeal
    self.mLeftHandle:setVisible(false)
    self.mRightHandle:setVisible(false)
    ...
    if tDeal.Seats == self.mMySeatID then
        ...
        --游戏开始时的动画
        dump(self.mPokerDeskType,"desktype")
        dump(self.mSelfRoomID,"self.mSelfRoomID")
        if self.mPokerDeskType == 4 and laixiaddz.LocalPlayercfg.LaixiaMatchlastStage ~= laixiaddz.LocalPlayercfg.LaixiaMatchStage then    --比赛场
                local system = laixiaddz.ani.CocosAnimManager
                self.secondstageAni = system:playAnimationAt(self.hPanelImageWidge,"doudizhu_secondstage")
                self.secondstageAni:setLocalZOrder(1005)
        ...
        self.mSelfPokers:setLocalZOrder(1006)
        self.mSelfPokers:setPosition(cc.p(640,self.mSelfPokers:getContentSize().height * 0.5+54))

        -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_CHANGE_BUYRECORDER_WINDOW,true);
    ...
    if self.mMySeatID ==(tDeal.Seats + 2) % 3 then
        ...
        self.mRightPlayerPoker:setLocalZOrder(1002)
    ...
    if self.mMySeatID ==(tDeal.Seats + 4) % 3 then
        ...
        self.mLeftPlayerPoker:setLocalZOrder(1002)
...
function CardTableDialog:dealBottomPokersFunction(aiDeal)
    ...
    if self.mMySeatID == aiDeal.Seats then
        ...
        laixiaddz.LocalPlayercfg.isDealBottomAni = true
...
function CardTableDialog:GamethebeFunction(msg)
                ...
                if thebeData.Chip == 0 then
                    if thebeData.Type == 0 then --不叫
                        if synchronousData.Players[i].Sex == 0 then
                            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_man_call_no)
                        else
                            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_call_no)
                        end
                    else
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_call_grab_no)
                    end
                elseif thebeData.Chip == 1 then
                    if synchronousData.Players[i].Sex == 0 then
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_man_fen_1)
                    else
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_woman_fen_1)
                    end
                elseif thebeData.Chip == 2 then
                    if synchronousData.Players[i].Sex == 0 then
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_man_fen_2)
                    else
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_woman_fen_2)
                    end
                elseif thebeData.Chip == 3 then
                    -- if 2 == synchronousData.RoomType then
                        if synchronousData.Players[i].Sex == 0 then
                            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_man_fen_3)
                        else
                            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_click_woman_fen_3)
                        end
                    -- else
                    --     laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_call_3)
                    -- end
                elseif thebeData.Chip == 4 then
                    if math.random(2) == 1 then 
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_call_grab)
                    else
                        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_call_grab_me)
                    end
                end
        end
    end
    ...
    if thebeData.Constraint == 1 then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"无人叫地主，随机一人成为地主") 
        self.mTimes:setString("1")          
    end
...
function CardTableDialog:GamePlayFunction()
    ...
    dump(laixiaddz.LocalPlayercfg.LaixiaOutCards,"laixiaddz.LocalPlayercfg.LaixiaOutCards")
    local outPokersData = laixiaddz.LocalPlayercfg.LaixiaOutCards or {}

    dump(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex,"laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex")
    local outPokersIndex = laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex or {}

    dump(outPokersData.Cards,"outPokersData.Cards")
    if #outPokersData.Cards > 0 and self.mMySeatID~=outPokersData.Seat then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_play)
    end
    if self.mSelfPokers ~= nil then
        self.mSelfPokers:setOutPokers(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex or {},laixiaddz.LocalPlayercfg.LaixiaOutCards or {})
    end
    ...
    local playWarning = function(num,cell1,cell2)
        if num == 2 and cell1 == false then
            cell1 = true
            if sex == 0 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_warning_last_two)
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_w_warning_last_two)
            end
        elseif num == 1 and cell2 == false then
            cell2 = true
            if sex == 0 then
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_warning_last_one)
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.EVENT_SOUND.ui_evevt_w_warning_last_one)
            end            
        end
    end

    dump(self.mMySeatID,"self.mMySeatID")
    dump(outPokersData.Seat,"outPokersData.Seat")
    ...
    if self.mMySeatID ==(outPokersData.Seat + 4) % 3 then
        ...
        if self.mSelfPokers ~= nil then --左边玩家最后一手牌时，自己手牌已经没有了
            ...
            local aiOutPokersIndex =clone(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex) or {}
            local aiOutPokers=laixiaddz.LocalPlayercfg.LaixiaOutCards or {}
             if     #laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards >0 then
                aiOutPokersIndex =clone(laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards)
            end
            self.mTips = l_NorLogicObject:tipsFunction(aiType,aiTable, aiOutPokersIndex,aiOutPokers,self.hLaiziCard)    --提示信息
        end
        --存在结算消息过来后出出牌消息（容错处理）
        local tmpData = laixiaddz.LocalPlayercfg.LaixiaOutCards 
        if  #self.mTips == 0 and   laixiaddz.LocalPlayercfg.LaixiaOutCards ~= nil and laixiaddz.LocalPlayercfg.LaixiaOutCards.count  ~= 2 and self.mSelfPokers ~= nil then
            self.mTipsTime  = 0
            self.mTipsTimeOn = 30
            if not self.mCancelTrusteeship:isVisible() then
                self.mTipsBiggerCard:setVisible(true)
            end
            self.mTipsBiggerCard:setLocalZOrder(1200)
            self.mSelfPokers:setTouchEnabled(false)
            self.mSelfPokers:setBlackColor()
        end
    end
...
function CardTableDialog:selectPlaySoundFunction(sex,aiOutType,aiCards,aiOutPokersData)
    ...
    elseif  laixiaddz.LocalPlayercfg.LaixiaOutCardsCount == 1 then
    ...
    elseif #aiCards ~= 0 then
        if  math.random(2) == 1 and ( self.mCardType == l_CardTypeObj.SINGLE_CARD or self.mCardType == l_CardTypeObj.DOUBLE_CARD or self.mCardType ~= l_CardTypeObj.THREE_CARD  ) then
            ...
            if  typeIndex == 1 then 
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_big_you_1"])
            elseif  typeIndex == 2 then 
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_big_you_2"])
            else
                laixiaddz.soundTools.playSound(laixiaddz.soundcfg.POKER_SOUND[SEX.."_big_you_3"])
            end
        end
    end
end
...
function CardTableDialog:showCardFunction(msg) 
    ...
    for i = 1,3 do
       if  self.mShowPoker.Seat == players[i].Seat then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_landlord_w_open_hand)
       end
       break
    end
...
function CardTableDialog:synchronousStepDataFunction(msg)
    ...
    dump(self.mSynchronousData,"self.mSynchronousData")
    if self.mSynchronousData ~= nil then
        local time = math.floor((self.mStageStep.Times - self.mSynchronousData.TimesTamp)/1000)
        self.mTime:setString(time)
        self.mLabelTime:setString(time)
...
function CardTableDialog:showPlayerInfo()
    ...
    local players = self.mSynchronousData.Players
    local bottomSeat = laixiaddz.LocalPlayercfg.LaixiaLandlordSeat;
    for index = 1,3 do
        if self.mMySeatID == players[index].Seat then              --我自己
            self.myPID = players[index].PID -- 玩家自己的pid
        end
...
function CardTableDialog:DisplayBottomCardFunction(aiData)
    ...
    if #self.mSynchronousData.BottomCards > 0 then
        ...
        self:onChangePlayerImgFunction(laixiaddz.LocalPlayercfg.LaixiaLandlordSeat)
    end
end
...
function CardTableDialog:showHandPokers()
    ...
    for k,v in ipairs(tPlayer) do
        if v.PID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
            ...
            laixiaddz.LocalPlayercfg.MatchGold = v.Coin    --短线重连时，赋值
            break
        end
    end
    ...
    for k,v in ipairs(tPlayer) do
        if self.mMySeatID == v.Seat then
            ...
            self.mSelfPokers:setLocalZOrder(1000+2)
            self.mSelfPokers:setPosition(cc.p(640, self.mSelfPokers:getContentSize().height * 0.5+54))
            self.mSelfPokers:setTouchEnabled(true)
            self.mSelfPokers:clearColor()
            self.mSelfPokers:setOutPokers(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex or {} ,laixiaddz.LocalPlayercfg.LaixiaOutCards or {})
...
function CardTableDialog:DisplayAndSortLaiziFunction(tPlayer)
    ...
    self.mSelfPokers:showMark()
    self.mSelfPokers:addTo(self.hPanelImageWidge)
    self.mSelfPokers:setLocalZOrder(1002)
    self.mSelfPokers:setPosition(cc.p(640, self.mSelfPokers:getContentSize().height * 0.5 + 54))
    self.mSelfPokers:setTouchEnabled(true)
    self.mSelfPokers:clearColor()
    self.mSelfPokers:setOutPokers(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex or {} ,laixiaddz.LocalPlayercfg.LaixiaOutCards or {})
...
function CardTableDialog:DisplayOutCardFunction()
    ...
    if laixiaddz.LocalPlayercfg.LaixiaOutCards ~= nil then
        self.mCardType = l_NorLogicObject:MatchCardTypeFunction(clone(laixiaddz.LocalPlayercfg.LaixiaOutCardsIndex),self.hLaiziCard,self.mCardType)
            --aiType    筛选的牌型
            --aiTable   筛选的表 默认是自己整个手牌
            --aiOutPokersIndex 最后时候出的有效牌值
            --aiOutPokers       出的牌值
...
function CardTableDialog:ChangeGoldFunction(msg)
    ...
    for index = 1, #deskResult.CSBalance do
        --更新任务信息
        if laixiaddz.LocalPlayercfg.LaixiaPlayerID == deskResult.CSBalance[index].PID then
   ...
   local reslutNum = function(aiNum,aiSeat,aiCurrSeat,isWin)
        local atlasNum
        if isWin == true then
            if aiCurrSeat == laixiaddz.LocalPlayercfg.LaixiaLandlordSeat then
                atlasNum = self.winNum:clone()
                if self.mSelfRoomID == 50  then
   ...
   for i = 1,3 do
        for j = 1,3 do
            if deskResult.CSBalance[i].PID == player[j].PID then
                if laixiaddz.LocalPlayercfg.LaixiaLandlordSeat == player[j].Seat then
...
function CardTableDialog:createResultDataFunction(data)
    ...
    for i = 1,3  do
        if self.myPID == data.CSBalance[i].PID then
            self.mGold:setString(laixiaddz.helper.numeralRules_2(data.CSBalance[i].CurrentGold))
        ...
        elseif self.rightPID == data.CSBalance[i].PID then
            ...
            self.mRightInfoGold:setString(laixiaddz.helper.numeralRules_2(data.CSBalance[i].CurrentGold))
        elseif self.leftPID ==data.CSBalance[i].PID then
           --self.mLeftInfoGold:setString(data.CSBalance[i].CurrentGold)
           self.mLeftInfoGold:setString(laixiaddz.helper.numeralRules_2(data.CSBalance[i].CurrentGold))
        end
    end
    ...
    for i = 1,3 do
        if self.mMySeatID == self.mSynchronousData.Players[i].Seat then
            ...
            if laixiaddz.LocalPlayercfg.LaixiaPlayerID == self.mBottomPID and islandlordWin == true then
                result.state  = 1
            elseif laixiaddz.LocalPlayercfg.LaixiaPlayerID == self.mBottomPID and islandlordWin == false then
                result.state  = 2
            elseif laixiaddz.LocalPlayercfg.LaixiaPlayerID ~= self.mBottomPID and islandlordWin == true then
                result.state  = 4
            elseif laixiaddz.LocalPlayercfg.LaixiaPlayerID ~= self.mBottomPID and islandlordWin == false then
                result.state  = 3
            end
    ...
    result.jixu = 0 --可以继续
    --关闭商城和超级礼包页面和购买记牌器页面
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_SHOP_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_SUPERGIFT_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_BUYRECORDER_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_USEINFO_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_SETUP_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_HELP_WINDOW)

    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_TALKING_WINDOW)
    ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_REDEEMGIFT_WINDOW)
   ...
   ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_TABLERESULT_WINDOW,result)
end
...
function  CardTableDialog:GameClearCardFunction(aiData)
    ...
    -- 托管初始化
    laixiaddz.LocalPlayercfg.LaixiaisTrusteeship = false
    laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards={}  ---对他进行置空处理防止，最后亮牌的时候有影响
    -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_CHANGE_BUYRECORDER_WINDOW,false);
    self.isPlay = false
    ...
    for index = 1,#aiData.data.AllClearPokers  do
        ...
        dump( aiData.data.AllClearPokers[index]," aiData.data.AllClearPokers[index]")
...
function CardTableDialog:OnRefreshUIGold(msg)
    ...
    for i,v in ipairs(data) do
       if v.playerID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
...
function CardTableDialog:GameDisplayBarStateFunction()
    ...
    dump(self.mStageStep,"self.mStageStep")
    if self.mStageStep == nil then
        for index = 1, 3 do
    ...
    --出牌阶段 去掉其他阶段字提示 
    --断线重连
    if (self.mLeftNum:getString() == "20" or self.mRightNum:getString() == "20" or( laixiaddz.LocalPlayercfg.mDeal ~= nil  and  #laixiaddz.LocalPlayercfg.mDeal.Cards == 20) ) and tStep == 3  then
        self.mLeftHandle:setVisible(false)
        self.mRightHandle:setVisible(false)
        self.mSelfHandle:setVisible(false)            
    end
    dump("CardTableDialog:GameDisplayBarStateFunction()-->tCurrentSeat = "..tCurrentSeat)
    dump("CardTableDialog:GameDisplayBarStateFunction()-->tSeatID = "..self.mMySeatID)
    if tCurrentSeat == self.mMySeatID then
        ...
        elseif   self.mTips ~=nil and #self.mTips == 0 and laixiaddz.LocalPlayercfg.LaixiaOutCards ~= nil and laixiaddz.LocalPlayercfg.LaixiaOutCards.count  ~= 2  then
...
function CardTableDialog:GameDisplayBarFunction(aiType)
    laixiaddz.logGame("CardTableDialog:GameDisplayBarFunction()-->type = "..aiType)
    if aiType == 0 then 
...
function CardTableDialog:onTrusteeshipFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
      if laixiaddz.LocalPlayercfg.LaixiaisTrusteeship == false then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
        
        --self:GetWidgetByName("Image_NoBiggerCard"):setVisible(false)
        local packet = l_PacketObj.new("CancelHosting", "play_card")
        packet:setValue("room_id", self.mRoomID)-- 写入包数据
        if self.hDeskID == nil  then
            return 
        end
        packet:setValue("table_id", self.hDeskID)-- 写入包数据
        if self.mCancelTrusteeship == sender then
            packet:setValue("mand_type", 0)-- 写入包数据
        else
            packet:setValue("mand_type", 1)-- 写入包数据
        end
        packet:setValue("pid",laixia.LocalPlayercfg.LaixiaPlayerID)
        packet:setValue("msg_id","cancel_trusteeship")
        -- laixiaddz.net.sendPacketAndWaiting(packet)
        laixiaddz.net:sendSocketPacket(packet)
        laixiaddz.LocalPlayercfg.LaixiaisTrusteeship = true
      end
    end
end
...
function CardTableDialog:CancelTrusteeship(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        dump(packet,"packet---cancel --5405")
        -- laixiaddz.net.sendPacketAndWaiting(packet)
        -- laixiaddz.net.sendPacketAndWaiting(packet)
        packet:setValue("msg_id","cancel_trusteeship")
        packet:setValue("pid",laixia.LocalPlayercfg.LaixiaPlayerID)
...
function CardTableDialog:checkRankFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_GOMATCHRANK_WINDOW,{from=1})
    end
end
...
function CardTableDialog:onGameShopFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        laixiaddz.net.sendPacket(CreateGoon)
    end
end
function CardTableDialog:onSet(sender, event)
    if (event == ccui.TouchEventType.ended) then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
    print("GameSetsWindow:onShow1")
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_SETUP_WINDOW,1)
...
function CardTableDialog:onTalkFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
        print("CardTableDialog:onTalkFunction")
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_TALKING_WINDOW)
    end
end

function CardTableDialog:closeFunction()
    -- if self.mSelfRoomID == 50 then
    --     if ((self.mSynchronousData==nil) or (self.isPlay==false and (#self.mSynchronousData.Players<3))) and self.mMasterID == laixiaddz.LocalPlayercfg.LaixiaPlayerID then
    --         local onSendDelTable = l_PacketObj.new("onSendJoinTable", _LAIXIA_PACKET_CS_CreateDelID)
    --         onSendDelTable:setValue("TableID", self.hDeskID );--self.hDeskID
    ...
    --         laixiaddz.net.sendPacket(goback) 
    --     end
    --     -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_HALL_WINDOW);   --返回大厅
    --     return
    -- else
    if self.mPokerDeskType and self.mPokerDeskType == 2 then
        -- 游戏场
        if self.isPlay == true then
            -- 游戏开始了 不能退
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"游戏开始了不能退出")
        else
            local stream = laixiaddz.Packet.new("EnterListRoom", "LXG_GAME_LIST")
            stream:setReqType("get")
            local function cb(data)
                if data.dm_error==0 and data.data~=nil then
                    local game_list = {}
                    local list = data.data.game_list
                    local match_enroll_id
                    for i=1,#list do
                        if self.mRoomID == list[i].match_id  then
                            match_enroll_id = list[i].match_enroll_id
                            break
                        end
                    end
                    local stream =  laixiaddz.Packet.new("gameTableExit", "LXG_GAME_EXIT")
                    stream:setReqType("post")
                    stream:setValue("match_id", self.mRoomID)
                    stream:setValue("match_enroll_id",match_enroll_id)
                    ...
                    laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event)
                        if event.dm_error == 0 then
                            ...
                            -- dump(event)
                            -- print("游戏中退出牌桌失败！",event.dm_error)
                            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,event.error_msg)
                        end 
                    end) 
                end
            end
        ...
        return
    elseif self.mPokerDeskType and self.mPokerDeskType == 4 then
        -- 比赛场
        if self.isPlay == true then
            -- 开牌桌 退比赛
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_CUEWORDSSELECT_WINDOW,
            {  
                OnLeftCallFun = function()
                    -- 确认离开比赛
                    local stream = laixiaddz.Packet.new("EnterListRoom", "LXG_MATCH_ENROLL_LIST")
                    stream:setReqType("get")
                    ...
                    local function cb(data)
                        ...
                        if data.dm_error==0 and data.data~=nil then
                            ...
                            for i =1,#data.data.match_list do
                                matchlist[1].rooms[i] = {}
                                if self.mRoomID == data.data.match_list[i].match_id then
                                    match_enroll_id = data.data.match_list[i].match_enroll_id
                                    break
                                end
                            end
                            if match_enroll_id then
                                -- 比赛场退报名
                                local stream =  laixiaddz.Packet.new("matchTableExit", "LXG_PLAYER_MATCH_EXIT")
                                stream:setReqType("post")
                                stream:setValue("match_ins_id", smatch_enroll_id)
                                stream:setPostData("uid", laixiaddz.LocalPlayercfg.LaixiaPlayerID)
                                laixia.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
                                    if event.dm_error == 0 then
                                        ...
                                        -- 托管通知服务器
                                        laixiaddz.LocalPlayercfg.LaixiaisTrusteeship = false
                                        self:onTrusteeshipFunction(nil, ccui.TouchEventType.ended)
            ...
            -- 没开牌桌 退报名
            local stream = laixiaddz.Packet.new("match_out", "LXG_MATCH_CANCEL_ENROLL")
            stream:setReqType("post")
            stream:setValue("match_id", self.mRoomID)
            stream:setValue("match_enroll_id",self.matchInsId)
            stream:setPostData("uid", laixiaddz.LocalPlayercfg.LaixiaPlayerID)
            laixiaddz.LocalPlayercfg.LaixiaMatchRoom = self.mRoomID
            local function cb(data)
...
function CardTableDialog:leavePokerDesk()
    if self.isShow then
        if self.isPlay == true then
            ...
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"牌局结束时才能退出")
        else
...
function CardTableDialog:onthebeFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        local packet = l_PacketObj.new("CGCallLandlord", "play_card")
        packet:setValue("room_id", self.mRoomID)
        -- 写入包数据
        packet:setValue("table_id", self.hDeskID)
        packet:setValue("type", 0)
        packet:setValue("chip", point)
        packet:setValue("pid",laixiaddz.LocalPlayercfg.LaixiaPlayerID)
        packet:setValue("msg_id","call_point")
        laixiaddz.net:sendSocketPacket(packet)
        self:ChangeTouchbarFunction(self.mThebeBar, false)
    end
end
...
function CardTableDialog:OnGameChuPaiFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        local aiOutPokers = laixiaddz.LocalPlayercfg.LaixiaOutCards or { }
        if #laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards > 0 then  -- 如果有收到的癞子牌，则取癞子牌（上家发送的）
            aiOutPokers = clone(laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards)
        end
        ...
        if #outPokers > 0 and  matchResult == true  then
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_play)    --在此处写出要替换的牌型数值和牌型
            ...
            laixiaddz.LocalPlayercfg.LaixiaLaiziReplaceCards = showPoker
            ...
            laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_error)
            self.mTipsTime = 0
            self.mTipsTimeOn = 2
            self.mTipsErrorCardType:setVisible(true)
            self.mTipsErrorCardType:setLocalZOrder(1200)
        end 
    end
end
...
function CardTableDialog:onPlayerInfoFunction(sender, event)
    if (event == ccui.TouchEventType.ended) then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
        if self.mPokerDeskType == 4 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"比赛中无法查看玩家信息")  
            return
        end
        if self.mSelfRoomID == 50 then
            return
        ...
        elseif sender == self.Image_UserInfo_Bg_02 then
            ...
            PID = laixiaddz.LocalPlayercfg.LaixiaPlayerID
        ...
        self:getUserInfo(PID)
    end
end

-- 通过pid获取角色信息
function CardTableDialog:getUserInfo(PID)
    -- 新协议 三合一
    local playerInfo = {}
    local stream =  laixiaddz.Packet.new("userPlayerInfo", "LXG_USER_INFOS")
    stream:setReqType("get")
    stream:setValue("id", PID)
    laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
        if event.dm_error == 0 then
            local data = event.data
            playerInfo.Name = data.nick or ""
            playerInfo.imgPath = data.portrait or ""
            playerInfo.ID = data.uid or 0
            playerInfo.Sex = data.sex or 0
            if data.cash_info then
                playerInfo.gold = data.cash_info.gold_coin or 0
            end 
            if data.match_count then
                playerInfo.winParam = data.match_count.probability or 0
                playerInfo.winNum = data.match_count.runnerup or 0 
            end 
            if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "CardTableDialog" then
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_USEINFO_WINDOW, playerInfo)
            end
        else
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"获取信息失败！")
        end 
    end)

    -- local stream =  laixiaddz.Packet.new("userPlayerInfo", "LXG_USER_INFO")
    -- stream:setReqType("get")
    -- stream:setValue("uid", PID)
    -- self.playerInfoData = {}
    -- laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
    --     local data1 = event.userinfo
    --     if event.dm_error == 0 then
    --         self.playerInfoData.Name = data1.nick or ""
    --         self.playerInfoData.imgPath = data1.portrait or ""
    --         self.playerInfoData.ID = data1.uid or 0
    --         local stream =  laixiaddz.Packet.new("userPlayerInfo1", "MEDUSA_CASH_ACCOUNT")
    --         stream:setReqType("get")
    --         stream:setValue("uid", data1.uid)
    --         laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
    --             local data = event
    --             if event.dm_error == 0 then
    --                 self.playerInfoData.gold = event.gold_coin or 0 
    --                 local stream =  laixiaddz.Packet.new("userPlayerInfo2", "LXG_COUNT_INFO")
    --                 stream:setReqType("get")
    --                 stream:setValue("uids", event.uid)
    --                 laixiaddz.net.sendHttpPacketAndWaiting(stream.key, stream, function(event) 
    --                     if event.dm_error == 0 then
    --                         if event.data then
    --                             self.playerInfoData.winParam = event.data.probability or 0
    --                             self.playerInfoData.winNum = event.data.runnerup or 0
    --                         else
    --                             self.playerInfoData.winParam = 0
    --                             self.playerInfoData.winNum = 0
    --                         end
    --                     else
    --                         self.playerInfoData.winParam = 0
    --                         self.playerInfoData.winNum = 0
    --                         print("LXG_COUNT_INFO===获取失败")
    --                         ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW,"获取信息失败！")
    --                     end  
    --                     if laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow == "CardTableDialog" then
    --                         ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_USEINFO_WINDOW, self.playerInfoData)
    --                     end
    --                 end)
    --             else
...
function CardTableDialog:OnbeginFunction(msg)
        ...
        if laixiaddz.LocalPlayercfg.mRoomData ~= nil then
            dump(laixiaddz.LocalPlayercfg.mRoomData,"laixiaddz.LocalPlayercfg.mRoomData")

            local mRoomInfo = laixiaddz.LocalPlayercfg.mRoomData[self.mRoomID]
            
            -- local roomID = laixiaddz.LocalPlayercfg:room_query_Id(self.mRoomID)
            -- local mRoomInfo = l_RoomDataMgr:query("roomid",self.mRoomID);
            dump(mRoomInfo,"mRoomInfo")
            if mRoomInfo then
                laixiaddz.LocalPlayercfg.LaixiaInGolds = mRoomInfo["MinGold"]
            end
            local deskResult = self.mDeskResult
            local changeGold = 0
            for index = 1, #deskResult.CSBalance do
                if laixiaddz.LocalPlayercfg.LaixiaPlayerID == deskResult.CSBalance[index].PID then
                    laixiaddz.LocalPlayercfg.MatchGold = deskResult.CSBalance[index].CurrentGold
        ...
        self.mCancelTrusteeship:setVisible(false)
        --继续游戏的话 现在改成这里处理逻辑了 --modify by 王天也 0525
...
function CardTableDialog:sendContinueGame()
    ...
    stream:setReqType("post")
    stream:setValue("match_id",self.mRoomID)
    stream:setValue("is_continuance",1)
    ...
    --TODo 游戏场暂时写死
        dump("roomID",self.mRoomID)
        local result = {}
        result.data = {}
        result.data.RoomID = self.mRoomID
        result.data.RoomType = 2
        if data.dm_error == 0 then
            laixiaddz.LocalPlayercfg.laixiaddzRoomID = self.mRoomID

            dump(laixiaddz.LocalPlayercfg.laixiaddzRoomID,"laixiaddz.LocalPlayercfg.laixiaddzRoomID")
            if laixiaddz.LocalPlayercfg.laixiaddzRoomID <12 and laixiaddz.LocalPlayercfg.laixiaddzRoomID >8 then
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_CONTINUEGOLD_LANDLORDTABLE_WINDOW, result)
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_CLEAR_DATELANDLORDTABLE_WINDOW, "继续游戏调用")
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_WAITTINGCARTOON_WINDOW)
            end
        else
             ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_FLOWWORDS_WINDOW, result.error_msg)
             ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_HALL_WINDOW)
        end
    end
    laixiaddz.net.sendHttpPacketAndWaiting(stream.key,stream,cb)
...
function CardTableDialog:OnSendContinueGameFunction()
    if self.nomoney then
        ...
        elseif self.mPokerDeskType ==2 then
            ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SEND_QUICKGAME_WINDOW,1)
        end
        self.nomoney = false
    else
        --根据用户的钱判断所处的房间id，并判断是否降级，如果降级显示飘字并弹出超级礼包页面x
        --self.isDownState  0:初始化 1 触发降级条件
        if self.isDownState == 0 then
            -- local roominfo = l_RoomDataMgr:query("roomid",laixiaddz.LocalPlayercfg:room_query_Id());
            local roominfo1 
            if self.mPokerDeskType ==1  then
            ...
            elseif self.mPokerDeskType ==2 then
                ...
                if laixiaddz.LocalPlayercfg.mRoomData ~= nil then
                    roominfo1 = laixiaddz.LocalPlayercfg.mRoomData[self.mRoomID]
                else
                    roominfo1 = l_RoomDataMgr:query("roomid",9);
                end
            elseif  self.mPokerDeskType == 0 then
                roominfo1 = l_RoomDataMgr:query("roomid",1);
            end

            dump(laixiaddz.LocalPlayercfg.MatchGold,"laixiaddz.LocalPlayercfg.MatchGold")
            dump(roominfo1,"roominfo1")
            if roominfo1["MinGold"] == nil then
                roominfo1["MinGold"] = roominfo1["minGold"] or 0
            end
            if  roominfo1 and laixiaddz.LocalPlayercfg.MatchGold>roominfo1["MinGold"] then
                self.isDownState = 1
                --self:GameSendLeaveRoomFunction()
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_ROOMDEMOTE_WINDOW,{roominfo.name,self.mRoomID,self.hDeskID})
            else
                ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_TABLERESULT_WINDOW)
...
function CardTableDialog:GameSendLeaveRoomFunction()
    ...
    -- -- laixiaddz.net.sendPacketAndWaiting(packet)
    -- laixiaddz.net:sendSocketPacket(packet)
    dump(self.isPlay ,"self.isPlay--GameSendLeaveRoomFunction")
    if self.isPlay == false then
        laixiaddz.LocalPlayercfg.laixiaddzisConnectCardTable = false
        dump(laixiaddz.LocalPlayercfg.laixiaddzJoinMatch,"laixiaddz.LocalPlayercfg.laixiaddzJoinMatch")
        if laixiaddz.LocalPlayercfg.laixiaddzJoinMatch == false then
...
function CardTableDialog:GameRefreshGoldFunction(msg)
    if self.isShow and (self.mPokerDeskType == 1 or self.mPokerDeskType == 2 or self.mPokerDeskType == 0) then
        self:GaibianShuziWenbenFunction(self.mGold,laixiaddz.LocalPlayercfg.MatchGold)
    end
...
function CardTableDialog:onRule(sender, event)
    if event == ccui.TouchEventType.ended then
        laixiaddz.soundTools.playSound(laixiaddz.soundcfg.BUTTON_SOUND.ui_button_open)
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_GAMEDESCRIBE_WINDOW,{data="bangzhu"})
    end
end

-- 更新当前排名
function CardTableDialog:updateRankInfo()
    if self.isShow and self.mRankingBG then
        ...
        local node = self:GetWidgetByName("Label_Current_Integral",self.mRankingBG)
        if node then
            node:setString(strInfo)
        end
    end
end

-- function CardTableDialog:showAppleDissmissLayer(data)
--     ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_APPLYDISMISS_WINDOW_WINDOW,data)
-- end
function CardTableDialog:onCallBackFunction()
    self:closeFunction()
end

function CardTableDialog:backHall()
    self:destroy()
end
 ...
 * @param  nil
--]]
function CardTableDialog:startTimer()
 ...
 * @param  nil
--]]
function CardTableDialog:stopTimer()

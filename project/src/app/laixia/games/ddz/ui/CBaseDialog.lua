...
local L_JT_window = laixiaddz.JsonTxtData:queryTable("window");
...
function CBaseDialog:Adaptation()
    ...
    S_XY(node, (display.width-1280)/2, 0)

    local layer = self.layer
    S_AXY(layer, 0.5, 0.5)
    S_XY(layer, display.cx, display.cy)
    layer:setScaleX(display.width/1260)

    local view = self.mInterfaceRes;
    S_AXY(view, 0.5, 0.5)
    S_XY(view, display.cx, display.cy)
...
function CBaseDialog:onCallBackFunction()
    ...
    if self:getName() == "LobbyWindow" then
        ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_SHOW_ANDROIDBACKTK_WINDOW)
    ...
    elseif self.hDialogType == DialogTypeDef.DEFINE_SINGLE_DIALOG and laixiaddz.LocalPlayercfg.OnReturnFunction then
    ...
    if(not self.mIsLoad) then
        if self.hDialogType ~= DialogTypeDef.DEFINE_INNER_DIALOG and self.hDialogType ~= DialogTypeDef.DEFINE_SINGLE_DIALOG then
            ...
            local function touchEnded(touch, event)
                if  laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow=="WonderfulWindow" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "DailyLoginWindow" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "TPFirstCharge" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "PersonalCenterWindow" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "GameSets" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "UserInfoWindow" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "StatinMessage" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "MyBagWindow" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "GameAbout" or laixiaddz.LocalPlayercfg.LaixiaCurrentNomarlWindow == "TPNoticeWindow" then
                    self:destroy()
                end    
            end
            ...
            --     -- ObjectEventDispatch:pushEvent(_laixiaddz_EVENT_HIDE_MARKEDWORDS_WINDOW)
            --     if self.layer then
            ...
            self.mInterfaceRes:setLocalZOrder(1001)
            self:Adaptation()
        else
            self.layer= display.newLayer()
...
function CBaseDialog:show(...)
    ...
    if(self.hDialogType == DialogTypeDef.DEFINE_SINGLE_DIALOG) then
        laixiaddz.LocalPlayercfg.laixiaddzCurrentWindow = self.mDialogName
    end
    if(self.hDialogType == DialogTypeDef.DEFINE_NORMAL_DIALOG) then
        laixiaddz.LocalPlayercfg.laixiaddzCurrentNomarlWindow = self.mDialogName

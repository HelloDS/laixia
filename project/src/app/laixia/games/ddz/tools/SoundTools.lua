...
function soundTools.soundOn()
      laixiaddz.LocalPlayercfg.LaixiaSoundOn = true
     -- --恢复背景音的播放
...
function soundTools.soundOff()
    laixiaddz.LocalPlayercfg.LaixiaSoundOn = false
...
function soundTools.playMusic(music, loop)
    ...
    --查看声音是否开启
    if(laixiaddz.LocalPlayercfg.LaixiaSoundOn == false) then 
        return
    end

    if(laixiaddz.LocalPlayercfg.LaixiaMusicValue == 2) then 
        return
    end
...
function soundTools.playSound(sound, delay)
    ...
    --查看声音是否开启
    if(laixiaddz.LocalPlayercfg.LaixiaSoundValue == 2) then 
        return
    end

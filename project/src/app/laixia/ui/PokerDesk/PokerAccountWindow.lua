...
function PokerAccountWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_TABLERESULT_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_TABLERESULT_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_IS_GAOONSELFBUILDING, handler(self, self.onRoomClose))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_DOWNLOAD_RESULT_SMALL_PICTURE_WINDOW, handler(self, self.headDownloadSuccess))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_GETLAIDOU,handler(self, self.GetLaidou))
end
...
function PokerAccountWindow:onShow(data)
   ...
   -- self.mTime = self:GetWidgetByName("Label_Time_Self")
    -- if laixia.LocalPlayercfg.LaixiaSelfBuildTable ~= nil and laixia.LocalPlayercfg.LaixiaSelfBuildTable > 0 then
    ...
    --
    if laixia.LocalPlayercfg.LaixiaSelfBuildTable ~= nil and laixia.LocalPlayercfg.LaixiaSelfBuildTable > 0 or self.info.gametype == 4 then
        fuwufei:setVisible(false)
    else
        local l_RoomDataMgr =  laixia.JsonTxtData:queryTable("room_list")
        ...
        cc.CallFunc:create(
            function()
                ...
                if self.isNeedShowLaidou~=nil then
                    self.itemData = laixia.JsonTxtData:queryTable("items")
...
function PokerAccountWindow:goBackFun(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if laixia.LocalPlayercfg.LaixiaSelfTotalInning ~= false then
            if laixia.LocalPlayercfg.LaixiaSelfInning  == laixia.LocalPlayercfg.LaixiaSelfTotalInning and laixia.LocalPlayercfg.LaixiaSelfTotalInning >0 then
                self:onRoomClose()
                return
            end
        end
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_EXITROOM_LANDLORDTABLE_WINDOW)
    end
end
...
function PokerAccountWindow:showBankruptcyBag()
    if laixia.LocalPlayercfg.LaixiaBankruptItemID ~= nil then
        local userGolds = self.selfInfo.haveGold
        local callBack = function()
            local checkCoins = laixia.JsonTxtData:queryTable("common"):query("key","limitMoney");
            if userGolds < checkCoins.Num then   --检测当前金是否低于阀值
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SEND_RELIEF_WINDOW)
            end
        end
        -- giftMoney 破产礼包阈值
        local giftMoney = laixia.JsonTxtData:queryTable("common"):query("key","giftMoney");
        if userGolds <= giftMoney.Num then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_BANKRUPTBAG_WINDOW,{OnCallFunc=callBack})    --请求破产礼包
        end
    end
end
    ...
    for i=1,3 do
        ---if i == 1  then
            if ui.CardTableDialog.mRoomID == 50  then -- 自建桌
                ...
                self:GetWidgetByName("Label_Player_Name",image_PokerResult):setString(laixia.helper.StringRules_6(playerInfo[i].Nickname))
...
function PokerAccountWindow:onRoomClose()
    ...
    obj.time = 10
    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_TISHIYUOTHER_WINDOW,obj)
end
...
function PokerAccountWindow:onTryAgain()
    ...
    if laixia.LocalPlayercfg.LaixiaSelfBuildTable ~= nil and laixia.LocalPlayercfg.LaixiaSelfBuildTable > 0 then
        ...
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_BEGIN_LANDLORDTABLE_WINDOW)
    end

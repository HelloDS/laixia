...
function PokerResultWindow_SelfBuilding:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_SELFBUILDINGRESULT_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_SELFBUILDINGRESULT_WINDOW, handler(self, self.destroy))
--    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SELFBUILDING_REWARDS, handler(self, self.getRewards))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_DOWNLOAD_RESULT_BIG_PICTURE_WINDOW, handler(self, self.onHeadDoSuccess))
end
...
function PokerResultWindow_SelfBuilding:getRewards(msg)
    ...
    self.Rewards.overFun  =  function() ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_HALL_WINDOW) end
...
function PokerResultWindow_SelfBuilding:onShow(msg)
    ...
    if ui.CardTableDialog.mSynchronousData == nil then
      ...
      for i=1,3 do
            ...
            self:GetWidgetByName("Label_Player_Name",image_PokerResult):setString(laixia.helper.StringRules_6(player[index].Nickname))
    ...
    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_UPDATEGOLD,uigold)          
    ...
    --:pushBackCustomItem(rankCell)
       ...
       -- if laixia.LocalPlayercfg.LaixiaPlayerID ~= playerID then
...
--              ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_GAIN_WINDOW, self.Rewards)
--              self.Rewards = {}
--        else

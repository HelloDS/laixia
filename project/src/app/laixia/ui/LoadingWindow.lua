...
function LoadingWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_LOADIN_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_GETVERSION,handler(self,self.getVersion))
    -- self.isclick = false
end
...
function LoadingWindow:showLogin()
    ...
    if device.platform ~= "windows" then
         -- laixia.LocalPlayercfg.PluginChannel =require("app.laixia.game.PluginChannel").create()
         -- laixia.LocalPlayercfg.PluginChannel:startSession()

         if laixia.LocalPlayercfg.CHANNELID ~= nil and laixia.LocalPlayercfg.CHANNELID ~= "" then
             if tonumber(laixia.LocalPlayercfg.CHANNELID) < 200000 then -- 渠道商店包
                 if tonumber(laixia.LocalPlayercfg.CHANNELID) == 160186 or tonumber(laixia.LocalPlayercfg.CHANNELID) == 111267 then --官网安卓和官网苹果包
                     --显示微信登录
             ...
             elseif tonumber(laixia.LocalPlayercfg.CHANNELID) >= 200000 and tonumber(laixia.LocalPlayercfg.CHANNELID) < 300000 then -- 自定义渠道包
                 if laixia.LocalPlayercfg.CHANNELID == 201010 then --映客渠道包 
             ...
             elseif tonumber(laixia.LocalPlayercfg.CHANNELID) >= 300000 then --苹果渠道包
                 --只显示微信登录
                 self:showWXLogin(2)
                 self.wxLogin:setVisible(true)
                 self.wxLogin:setPositionX(self.x_)
    ...
    if eventType == ccui.TouchEventType.ended then
        function JoinGameCallFunc(sign)
            if sign == true then
                ...
                --登录失败
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, { Text = "登陆失败"})
            end
        end
        laixia.LocalPlayercfg.PluginChannel =require("app.laixia.game.PluginChannel").create()
        laixia.LocalPlayercfg.PluginChannel:setEndCallBack(JoinGameCallFunc)
        laixia.LocalPlayercfg.PluginChannel:login() 
    end
end
...
function LoadingWindow:showWXLogin(poss)
    ...
    if laixia.LocalPlayercfg.CHANNELID~=nil and ((tostring(laixia.LocalPlayercfg.CHANNELID) == "160186" or tostring(laixia.LocalPlayercfg.CHANNELID) == "111267") or (tonumber(laixia.LocalPlayercfg.CHANNELID)>200000 and tonumber(laixia.LocalPlayercfg.CHANNELID)<300000)) then
        if laixia.kconfig.isYingKe == true then
        else
            local system = laixia.ani.CocosAnimManager
            self.yingke_entry = system:playAnimationAt(self.LoginPanel,"yingke_entry")
...
--    local weixinLogin = laixia.Layout.loadNode("entry.csb"):addTo(self.LoginPanel)
...
--        ))
    ...
    if cc.UserDefault:getInstance():getBoolForKey("isauto") == true or (cc.UserDefault:getInstance():getStringForKey("access_token")~="" and cc.UserDefault:getInstance():getStringForKey("openid")~="") and self.isChange~=true then
          if PLATFORM_OS_ANDROID == targetPlatform then
            ...
            laixia.LocalPlayercfg.LaixiaUserID = openid
            laixia.LocalPlayercfg.LaixiaTokenID = access_token
...
local function luaWeChatfun(param)
    ...
    laixia.LocalPlayercfg.HEAD_URL = array.headimgurl
    laixia.LocalPlayercfg.LaixiaPlayerGender = array.sex - 1
...
function LoadingWindow:weChatLogin(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"请勾选同意下方的\"服务协议\"，即可进入游戏哦") 
        --     return
        -- end     
        -- funToWeChatJava()
         if (cc.PLATFORM_OS_ANDROID == targetPlatform) then
    ...
    local function onResp()
        if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
            ...
            if respJson.errcode then
                print("拉起失败1")
                ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, {Text = "你没有安装微信！"})
                -- 申请失败
                --require("app/views/NoticeTips"):create(gt.getLocationString("LTKey_0007"), gt.getLocationString("LTKey_0030"), nil, nil, true)
            else
                 print("wangtianye   成功")
                -- laixia.LocalPlayercfg.LaixiaTokenID = respJson.token
                -- laixia.LocalPlayercfg.HEAD_URL = respJson.headimgurl
                -- laixia.LocalPlayercfg.LaixiaPlayerGender = respJson.sex - 1

                laixia.LocalPlayercfg.LaixiaTokenID = respJson.access_token
                print(laixia.LocalPlayercfg.LaixiaTokenID)
    ...
    local function onResp()
        if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
            ...
            if respJson.errcode then
                ...
                laixia.LocalPlayercfg.LaixiaUserID = respJson.openid
                laixia.LocalPlayercfg.LaixiaTokenID = access_token
                laixia.LocalPlayercfg.LaixiaPlayerNickname = respJson.nickName
                laixia.LocalPlayercfg.HEAD_URL = respJson.headimgurl
                laixia.LocalPlayercfg.LaixiaPlayerGender = respJson.sex - 1
...
function LoadingWindow:onShow(data)
    ...
    if laixia.kconfig.isYingKe == true then
        ...
        laixia.LocalPlayercfg.LaixiaMusicValue = cc.UserDefault:getInstance():getIntegerForKey("MusicValue")
        laixia.LocalPlayercfg.LaixiaSoundValue = cc.UserDefault:getInstance():getIntegerForKey("SoundValue")
        laixia.LocalPlayercfg.LaixiaIsShake = cc.UserDefault:getInstance():getIntegerForKey("Shake")   
        ...
        laixia.LocalPlayercfg.mCurGameAppID =1 
...
--        if laixia.config.isOpenUpdate and device.platform~="windows" and laixia.config.isAudit~=true then
            ...
            laixia.ui.WindowManager:onRegisterAll()
        ...
        self.loadingNode = self:GetWidgetByName("loadingNode")
        self.loadingNode:setVisible(false)
        --laixia.LocalPlayercfg.LaixiaIsShake = true 
        --读取本地的设置，音量音乐，震动等

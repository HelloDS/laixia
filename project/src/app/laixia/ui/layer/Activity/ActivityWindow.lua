...
function Activity:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_ACTIVITY_WINDOW,handler(self,self.show))
      --下载图片完成
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_DOWNLOAD_ACTIVITY_WINDOW, handler(self, self.onActivityDoSuccess))
end
...
function Activity:onShow(data)
    ...
    if laixia.LocalPlayercfg.AdvermentIndex ~=nil then
        for k,v in pairs(self.ActivityName) do  
            -- local pathStrTemp = laixia.LocalPlayercfg.mPathArray[laixia.LocalPlayercfg.AdvermentIndex or 1]
...
function Activity:onActivityDoSuccess(msg)
    ...
    if (fileExist) then
        if isTableEmpty(laixia.LocalPlayercfg.LaixiaActivityPath) then
            if laixia.LocalPlayercfg.LaixiaActivityPath[index]==nil then
                laixia.LocalPlayercfg.LaixiaActivityPath[index] = {}
            end
            laixia.LocalPlayercfg.LaixiaActivityPath[index] = activityInUse
            --table.insert(laixia.LocalPlayercfg.LaixiaActivityPath,activityInUse)
        else
            local tagPath = 0
            for k,v in pairs(laixia.LocalPlayercfg.LaixiaActivityPath) do
...
function Activity:addPathArray(pathArray ,ActivityListData,ActivityListData_1,ActivityButtonArray,ActivityName)
        ...
        for i,v in pairs(laixia.LocalPlayercfg.LaixiaActivityPath) do     
            -- if (activityValue.channelID~=nil and activityValue.channelID == tostring(laixia.LocalPlayercfg.CHANNELID)) or activityValue.channelID==nil then
...
--            button_FunWenzi:loadTexture(self.ActivityButtonArray[mIndex])
        ...
        elseif self.ActivityName[self.mIndex] == "iPhonextuiguang2" then
...
function Activity:goShareFriend(sender, eventType)
    if eventType == ccui.TouchEventType.ended then 
        if laixia.kconfig.isYingKe == false then
            ...
            local url = "http://wx.laixia.com/ma?a=share&user_id=".. laixia.LocalPlayercfg.LaixiaPlayerID .."&token="..token
        ...
        elseif laixia.kconfig.isYingKe == true then
            ...
            if device.platform == "ios" then
                ...
                local args = {
                                ...
                                image = "http://img2.inke.cn/MTUxNTE1OTgzNzQ2MiMxNTcjanBn.jpg",
                }

...
function TPRetrievePassWord:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_FINDPWD_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_FINDPWD_WINDOW, handler(self, self.destroy))
end
...
function TPRetrievePassWord:sendFindPswPacket(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if string.len(self.verification:getString())  < 4 then
           ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW,"请输入4位验证码！")
            return
        end
      
        local CSFindPwd = Packet.new("CSCodeBinding",_LAIXIA_PACKET_CS_FindPwdID)      
        CSFindPwd:setValue("Rdf",laixia.LocalPlayercfg.LaixiaHttpCode) 

...
function TPFirstCharge:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_FIRSTGIFT_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SENDPACKET_FIRSTGIFT, handler(self, self.sendToSever))
end

function TPFirstCharge:sendToSever()
    local CSFirstSuperBag = Packet.new("CSFirstSuperBag", _LAIXIA_PACKET_CS_FirstSuperBagID)
    CSFirstSuperBag:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
...
--    local isFirst = data.FirstBuy
    ...
    local goods = laixia.JsonTxtData:queryTable("items"):query("ItemID",self.mItemID)
...
--    end
    ...
    -- end
    -- if isFirst < 0 then 
    --       ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, { Text = "订单处理中，请稍后再试！", OnCallFunc = function() self:destroy(); end })
    --     return  
    -- end

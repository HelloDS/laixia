...
function TPExtensionWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_EXTENSION_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_EXTENSION_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_UPDATE_EXTENSION, handler(self, self.onUpdataButton))
end
    ...
    if eventType == ccui.TouchEventType.ended then
        ...
        if str == "领取(0)" then
           if self.bindPid  ~= 0 then
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW, "暂无可领取金币") 
           else
                self:onBindPID()
           end
        else
...
function TPExtensionWindow:onUpdataButton(data)
    ...
    if AwardGold ~= 0 then
       ...
       ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW, "绑定成功，获得"..AwardGold.."金币") 
    else
       self.mReceiveTitle:setString("绑定")
    end
    laixia.LocalPlayercfg.LaixiaPlayerGold = data.data.AllGold
end
...
function TPExtensionWindow:onBindPID()
      ...
      if playerID == 0  or playerID == nil  then
        ...
        CSExtensionBinding:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
...
function TPExtensionWindow:onReceiveGold()
        ...
        CSReceiveExtension:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)

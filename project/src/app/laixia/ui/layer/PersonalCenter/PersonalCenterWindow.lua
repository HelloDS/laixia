...
function PersonalCenterWindow:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_LOBBYDETAILS_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SUCCESS_REVISESEXNICK_WINDOW, handler(self, self.ChangeNameSuccess))
...
function PersonalCenterWindow:onShowExtension(sender, event)
    if (event == ccui.TouchEventType.ended) then
        ...
        CSExtensionRule:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
...
--        self:AddWidgetEventListenerFunction("CheckBox_PersonalCenter_Sex2", handler(self,self.isWoman))
        ...
        if laixia.LocalPlayercfg.LaixiaPhoneNum == "" then
            ...
            self.Label_PersonalCenter_BingPhone:setString("已绑定手机号"..laixia.LocalPlayercfg.LaixiaPhoneNum)
...
--         end
--         if laixia.LocalPlayercfg.LaixiaPlayerLevel == 1 then
--             mNowExp = 0
--         else
...
-- -------------------------------------------------------------------------------
        ...
        if laixia.config.isAudit then
            self.PlayerID:setString("用户编号:"..laixia.LocalPlayercfg.LaixiaPlayerID)
        else
            self.PlayerID:setString("用户ID:"..laixia.LocalPlayercfg.LaixiaPlayerID)
        end
        -- --玩家称号
        -- self.PlayerTitle = self:GetWidgetByName("Label_PersonalCenter_ChV")
        -- self.PlayerTitle:setString(levelData[laixia.LocalPlayercfg.LaixiaPlayerLevel].GradeTitle)
        ...
        -- self.labelBeiShu:setString(laixia.LocalPlayercfg.LaixiaPlayerMaxWintimes)
        --  --等级
        -- self.Level = self:GetWidgetByName("Label_PersonalCenter_DjV")
        -- local userLevel = laixia.LocalPlayercfg.LaixiaPlayerLevel
...
-----------------------------------------------------------------------------------------------      
        ...
        if type(laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes) ~= "number" then
            laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes = 0 
        end

        if type(laixia.LocalPlayercfg.LaixiaPlayerFailureTimes)~= "number" then
            laixia.LocalPlayercfg.LaixiaPlayerFailureTimes = 0 
        end
        local totalJuShu = laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes + laixia.LocalPlayercfg.LaixiaPlayerFailureTimes 
...
---add by wangtianye
        ...
        self.Text_shenglifushu:setString(laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes )
        ...
        if totalJuShu == 0 or totalJuShu==nil then
            ...
            self.Text_youxichangshenglv:setString(string.format("%d",laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes/totalJuShu*100).."%" )
        end
        ...
        self.Text_zongcansaicishu:setString(laixia.LocalPlayercfg.LaixiaBisaiNum)
        ...
        self.Text_guanjuncishu:setString(laixia.LocalPlayercfg.LaixiaBisaiWin)
        self.Text_guanjuncishu:enableOutline(cc.c4b(107,62,27,255),2)
        self.Text_yajuncishu = self:GetWidgetByName("Text_yajuncishu")
        self.Text_yajuncishu:setString(laixia.LocalPlayercfg.LaixiaBisaiSecond)
        ...
        if laixia.kconfig.isYingKe == true then
            ...
            self:GetWidgetByName("Text_name_new"):setString(laixia.LocalPlayercfg.LaixiaPlayerNickname)

            self:GetWidgetByName("Text_126"):setString(laixia.LocalPlayercfg.ZhiShiBiNum)
            self:GetWidgetByName("Text_125"):setString(laixia.LocalPlayercfg.LaixiaPlayerGold)
            self:GetWidgetByName("Text_124"):setString(laixia.LocalPlayercfg.LaixiaPlayerGiftCoupon)

            self:GetWidgetByName("Text_133"):setString(totalJuShu)
            self:GetWidgetByName("Text_133_Copy_1"):setString(laixia.LocalPlayercfg.LaixiaBisaiNum)
            self:GetWidgetByName("Text_133_Copy"):setString(laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes)
            self:GetWidgetByName("Text_133_Copy_2"):setString(laixia.LocalPlayercfg.LaixiaBisaiWin)
            if totalJuShu == 0 or totalJuShu == nil then
                self:GetWidgetByName("Text_133_Copy_0"):setString("0")
            else
                self:GetWidgetByName("Text_133_Copy_0"):setString(string.format("%d",laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes/totalJuShu*100).."%" )
            end
            self:GetWidgetByName("Text_133_Copy_3"):setString(laixia.LocalPlayercfg.LaixiaBisaiSecond)

            ---这里先读id
            self:GetWidgetByName("Label_PersonalCenter_YhidV"):setString("芝士号:"..laixia.LocalPlayercfg.LaixiaUserID)
...
--        else 
--            local shenglv = math.floor(laixia.LocalPlayercfg.LaixiaPlayerVictoryTimes * 100 / totalJuShu)
--            self.labelShenglv:setString(shenglv .. "%")
--        end
        
        if laixia.LocalPlayercfg.LaixiaLastLoginPlatform == 1 then
        ...
        elseif laixia.LocalPlayercfg.LaixiaLastLoginPlatform == 2 then
            self.LoginType:setString("账号登陆:手机登陆")
        elseif laixia.LocalPlayercfg.LaixiaLastLoginPlatform == 5 then
        ...
        -- elseif 2 == laixia.LocalPlayercfg.LaixiaLastLoginPlatform  then  --如果手机登录
...
function PersonalCenterWindow:goPerson(sender,eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        PersonBill:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
...
function PersonalCenterWindow:sendChangeMsgPacket(ModifyType)
        ...
        if ModifyType == 1 then
            if nickLen > 16 or nickLen < 6 then
                --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "昵称长度为3-8个汉字或6-16个字符")  
                 ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"昵称长度为3-8个汉字或6-16个字符")
                return 
            end
            if nickLen <= 0 then
                --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "昵称不能为空")  
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"昵称不能为空")
                return 
            end
        end
        -- if self.mSignStrLen<=0 then
        --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "签名不能为空") 
        --      ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"签名不能为空") 
        --     return
        if ModifyType == 2 then
            if self.mSignStrLen>40 then
                --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "签名长度为0~20个汉字")  
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"签名长度为0~20个汉字") 
                return
            end
...
function PersonalCenterWindow:updateHead()
    ...
    if self.mIsShow then
        ...
        -- 默认头像图片路径
        self.rankIcon = {}
        local icon = laixia.LocalPlayercfg.LaixiaPlayerHeadUse
        ...
        --if icon ~= nil and icon ~= "" then
            ...
            if (fileExist) then
                ...
                DownloaderHead:pushTask(laixia.LocalPlayercfg.LaixiaPlayerID, netIconUrl,4)
            end
        --else
            self:addHeadIcon(self.Image_HeadBg,path)
        --end
...
--      end
    if mIndex == 1 then
        ...
        laixia.LocalPlayercfg.LaixiaPlayerGender = 0
...
function PersonalCenterWindow:ShowNameAndSex()
    self.PlayerName:setString(laixia.LocalPlayercfg.LaixiaPlayerNickname)
    if laixia.LocalPlayercfg.LaixiaPlayerGeXingqianming == "" then 
        self.LabelSignature:setString("记录此刻的心情")
    else
        self.LabelSignature:setString(laixia.LocalPlayercfg.LaixiaPlayerGeXingqianming)
        self.NewSignature:setText(laixia.LocalPlayercfg.LaixiaPlayerGeXingqianming)
    end
    self.labelNewName:setText(laixia.LocalPlayercfg.LaixiaPlayerNickname)
    if 0 == laixia.LocalPlayercfg.LaixiaPlayerGender then
        self.sex = 0
...
function PersonalCenterWindow:ChangeNameSuccess()
   ...
   elseif self.Button_Xiugaiqianming:isVisible() == false then
    ...
    laixia.LocalPlayercfg.LaixiaPlayerGender = self.sex
    laixia.LocalPlayercfg.LaixiaPlayerNickname = self.mTempName 
    laixia.LocalPlayercfg.LaixiaPlayerGeXingqianming = self.mSignStr
    --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_DUIHUAN_SHOW_WINDOW, "保存成功!")
    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"保存成功!")
    self:ShowNameAndSex()
end
...
local function luafun(param)
   if nil == param then
    ...
    laixia.LocalPlayercfg.LaixiaPlayerHeadPicture = param
    laixia.LocalPlayercfg.LaixiaIsChangeHead = true
end
...
function PersonalCenterWindow:Button_PersonalCenter_Xgmm(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        if  laixia.LocalPlayercfg.LaixiaLastLoginPlatform ~= 2 then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, "请先绑定手机")   
        else
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_REVISEPASSWD_WINDOW)
            self:onClose()
        end
    end
...
function PersonalCenterWindow:switchAccount(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        laixia.soundTools.playSound(soundConfig.BUTTON_SOUND.ui_button_open)
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_ACCOUNTLOGIN_WINDOW)
        self:onClose()
    end
end
...
function PersonalCenterWindow:onTick(dt)
    ...
    if laixia.LocalPlayercfg.LaixiaPhoneNum ~="" then
        self:GetWidgetByName("Button_PersonalCenter_Bdsj"):setVisible(false)
        self:GetWidgetByName("Text_11"):setVisible(false)
        self:GetWidgetByName("Label_PersonalCenter_BingPhone"):setString("已绑定手机号"..laixia.LocalPlayercfg.LaixiaPhoneNum)
    end

    if laixia.LocalPlayercfg.LaixiaPhoneNum ~="" and laixia.kconfig.isYingKe == true then
        self:GetWidgetByName("Button_107"):setVisible(false)
        self:GetWidgetByName("Text_123"):setString("已绑定手机号"..laixia.LocalPlayercfg.LaixiaPhoneNum)
    end

    self.PlayerJiangQuan:setString(tostring(laixia.LocalPlayercfg.LaixiaPlayerGiftCoupon))
    self.PlayerGold:setString(tostring(laixia.LocalPlayercfg.LaixiaPlayerGold))


    if laixia.LocalPlayercfg.LaixiaIsChangeHead then
        if (laixia.LocalPlayercfg.LaixiaPlayerHeadPicture ~= nil) and(laixia.LocalPlayercfg.LaixiaPlayerHeadPicture ~= "") then
            -- 用户自定义头像
            local path
            if string.find(laixia.LocalPlayercfg.LaixiaPlayerHeadPicture,".png") then
                path = cc.FileUtils:getInstance():getWritablePath() .. laixia.LocalPlayercfg.LaixiaPlayerHeadPicture 
            else
                path = cc.FileUtils:getInstance():getWritablePath() .. laixia.LocalPlayercfg.LaixiaPlayerHeadPicture..".png"
            end
        ...
        laixia.LocalPlayercfg.LaixiaIsChangeHead = false
    end
end
...
function PersonalCenterWindow:doUpload(imgPath)
    ...
    CSHeadUpload:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)

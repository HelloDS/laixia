...
function GameListWaitting:onInit()
    self.super:onInit(self) 
     ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_WAITSTATE_WINDOW, handler(self, self.show))
     ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_WAITSTATE_WINDOW, handler(self, self.destroy))
end
...
--    dizhupao:setLocalZOrder(10000)
    ...
    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_MATCHWAITLOADING_WINDOW)-- 删掉中间等待界面
    ...
    local mMatchName=""
    if laixia.LocalPlayercfg.LaixiaMatchName == "" or laixia.LocalPlayercfg.LaixiaMatchName == nil  then
        mMatchName = cc.UserDefault:getInstance():getStringForKey("MatchName")
    else
        mMatchName = laixia.LocalPlayercfg.LaixiaMatchName
    end
...
function GameListWaitting:onTick(dt)
    if self.mIsLoad == true then
        --if laixia.LocalPlayercfg.LaixiaMatchShowbar == true then
       ...
       -- else
       --     self.ChangeTxt:setString(laixia.LocalPlayercfg.LaixiaMatchRank .. "/" .. laixia.LocalPlayercfg.LaixiaMatchTotalNum)
       -- end
...
function GameListWaitting:onDestroy()
	 laixia.LocalPlayercfg.LaixiaMatchShowbar =false
end

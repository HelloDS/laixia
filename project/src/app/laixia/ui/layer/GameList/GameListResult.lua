...
function GameListResult:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MATCHRESULT_WINDOW,handler(self,self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_MATCHRESULT_WINDOW,handler(self,self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_CONTINUEMATCH_WINDOW,handler(self,self.SendMatchPacket))
    --失败 没有奖励
...
function GameListResult:onShow(mesg)
    ...
    self.root:runAction(cc.Sequence:create(
                ...
                cc.CallFunc:create(
                    function()   
                        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_CLEAR_DATELANDLORDTABLE_WINDOW,"结算清除数据")
                    end),nil))  
    ...
    if GameType == 2 then--self.mData.GameType == 2 then
    ...
    if self.mData.rewards ~= nil then
        ...
        self.promotion:setLocalZOrder(5)
        ...
        laixia.soundTools.playSound(soundConfig.EVENT_SOUND.ui_evevt_bear_palm)
        laixia.LocalPlayercfg.LaixiaMatchRank = 0
        laixia.LocalPlayercfg.LaixiaMatchTotalNum = 0
        ...
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_MATCHRANK_WINDOW)
        ...
        local resultStr = ""
        if laixia.LocalPlayercfg.LaixiaMatchName == nil or laixia.LocalPlayercfg.LaixiaMatchName=="" then
            resultStr = cc.UserDefault:getInstance():getStringForKey("MatchName")
        else
            resultStr = laixia.LocalPlayercfg.LaixiaMatchName
        end
        ...
        CGEnterLobby:setValue("Code", laixia.LocalPlayercfg.LaixiaHttpCode)
        ...
        self:GetWidgetByName("Label_GamePrize_Name"):setString(laixia.LocalPlayercfg.LaixiaPlayerNickname) --玩家昵称
        ...
        if #match == 0 then
            MatchName = laixia.LocalPlayercfg.LaixiaMatchName
        else
        ...
        local  renshu =laixia.LocalPlayercfg.LaixiaMatchLimit
        if renshu ==0  then
          renshu = "N"
        end
        --奖励
        ...
        if self.mData.rank <= 3 then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW, "比赛结束，恭喜您获得好成绩！")
            -- 报名成功飘字
        end
        ...
        for i, v in pairs(self.mData.rewards) do
            if string.sub(tostring(v.ItemID),1,2) == "15" then
                ...
                redbagNum = redbagNum + tonumber(string.split(Itemsdata.ItemName,"元红包")[1])
            end
        end
        if redbagNum == 0 then
        ...
        --self:GetWidgetByName("Label_GamePrize_Title"):setVisible(false)
        -- self:GetWidgetByName("AtlasLabel_Num" ,cell):setVisible(false)
        -- self:GetWidgetByName("Image_JPIcon" ,cell):setVisible(false)
        print("播放动画+++++++++++++++++++++++++++++++++++++++++++++++")
        local resultStr = ""
        if laixia.LocalPlayercfg.LaixiaMatchName == nil or laixia.LocalPlayercfg.LaixiaMatchName=="" then
            resultStr = cc.UserDefault:getInstance():getStringForKey("MatchName")
       ...
       -- self.addAnimation = self:GetWidgetByName("Node_addAnimation")
        local system = laixia.ani.CocosAnimManager
        self.eliminate = system:playAnimationAt(self.Image_Faile,"doudizhu_eliminate")
        self.eliminate:setLocalZOrder(5)
...
function GameListResult:onTick()
...
function GameListResult:SendMatchPacket()
        ...
        baoming:setValue("PageType", 1)
        baoming:setValue("RoomID", laixia.LocalPlayercfg.LaixiaMatchRoom)
        baoming:setValue("ItemID", laixia.LocalPlayercfg.LaixiaJoinGame)
        laixia.net.sendPacket(baoming)
...
function GameListResult:goShare(sender, eventType)
    if ccui.TouchEventType.ended == eventType then
        ...
        laixia.LocalPlayercfg.MatchGold = 0
        -- 只有有奖励的时候才会出现分享的图标
        if self.mData.rewards ~= nil then

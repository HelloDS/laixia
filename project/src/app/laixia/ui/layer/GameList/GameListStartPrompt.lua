...
function GameListStartPrompt:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MATCHFLICKERWORDS_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_ADD_MATCHFLICKERWORDS_WINDOW,handler(self,self.addMesg))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_MATCHFLICKERWORDS_WINDOW, handler(self, self.destroy))
end
...
function GameListStartPrompt:onShow(msg)
    if  msg.data == nil then
        if self.mesg == "" or self.mesg == nil then
            ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_MATCHFLICKERWORDS_WINDOW)
        end
    else

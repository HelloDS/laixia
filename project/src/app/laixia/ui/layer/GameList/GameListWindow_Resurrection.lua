...
function GameListWindow_Resurrection:onInit()
    ...
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MATCHEASTER_WINDOW,handler(self,self.show))--会首先调用父类的show方法生成界面
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_MATCHEASTER_WINDOW,handler(self,self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_MATCHEASTERRESULT_WINDOW,handler(self,self.goMatchresult))-- 复活失败显示结算界面
...
function GameListWindow_Resurrection:onShow(mesg)
    ...
    for i, v in pairs(Items) do
           ...
           if v.ItemID == 1001 then
              laixia.LocalPlayercfg.LaixiaInGolds=v.ItemCount  --复活需要金数量

...
function GameListRanking:onInit()
    ...
    -- 注册事件
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_MATCHRANK_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_HIDE_MATCHRANK_WINDOW, handler(self, self.destroy))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_GOMATCHRANK_WINDOW,handler(self,self.goGameListRanking))
end
...
function GameListRanking:onShow(mesg)
    ...
    if  laixia.LocalPlayercfg.LaixiaIsShowMatchRank == true then  -- 如果显示比赛排行
...
function GameListRanking:addRank(begin,over)
    if self.mIsLoad == false then
     ...
     local v = self.data[i]
        ...
        print(v.Coin)
        local model = nil
        if laixia.LocalPlayercfg.LaixiaPlayerNickname == v.NickName then --突出自己
...
function GameListRanking:onTick()
    if self.mIsLoad == true and laixia.LocalPlayercfg.LaixiaIsShowMatchRank == true then
        if #self.data == 0 then
            return
        end
        local temp = 10

...
function MyBagWindow:onInit()
    ...
    self.mIsShow = false
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_SHOW_TOOLSBOX_WINDOW, handler(self, self.show))
    ObjectEventDispatch:addEventListener(_LAIXIA_EVENT_UPDATE_TOOLBOX_WINDOW,handler(self,self.updateWindow))
end
--点击查看详情
function MyBagWindow:onClickToShow(sender, eventtype)
    ...
    elseif eventtype == ccui.TouchEventType.ended then
        ...
        if sender.ItemID == 13002 then
            ...
            if laixia.LocalPlayercfg.LaixiaTaskList ~= nil then
                for k,v in pairs(laixia.LocalPlayercfg.LaixiaTaskList) do
    ...
    if eventtype == ccui.TouchEventType.ended then  
        ...
        if self.CheckBox_duihuan:isSelected() == true then  
            ...
            if itemMsg.isDisapear == 1 then
                ...
                -- laixia.LocalPlayercfg.LaixiaPlayerGold = laixia.LocalPlayercfg.LaixiaPlayerGold + self.duihuanNum
        elseif self.CheckBox_hongbao:isSelected() == true then
            if laixia.LocalPlayercfg.LaixiaPhoneNum=="" then
                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"请点击游戏头像进行手机绑定")
                self.Panel_3:setVisible(false)
                self.isTouched = false
                return
            end
           ...
           --local pid = crypto.md5(sender.ItemID)
            ...
            jc:registerScriptHandler( function() 
                ...
                if array1.code>0 then --xhr.code >0代表此用户信息存在 <0此用户信息不存在 msg代表具体原因
                    print(array1.msg)
                    --第一阶段成功
                    --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,array1.msg)
                    --TIPS
                    ...
                    xhr:registerScriptHandler( function() 
                        ...
                        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MONEY_WINDOW,array2.msg)
                        if array2.code == 1 then
                            print("使用成功")
                            -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                             -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"使用成功!")
                            -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MONEY_WINDOW,"红包使用成功，请前往公众号领取!")
                    ...
                    local accessTokenURL = string.format("http://wx.laixia.com/posthb?unionid=%s&propid=%s&objectid=%s&phone=%s",unionid,pid,sender.ItemObjectID,laixia.LocalPlayercfg.LaixiaPhoneNum)
                    xhr:open("GET", accessTokenURL,true)
                    xhr:send()
                else
                    print(array1.msg)
                    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MONEY_WINDOW,"请关注微信服务号:" .. laixia.LocalPlayercfg.LaixiaWechatServiceNum)
                    self.Panel_3:setVisible(false)
                    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,array1.msg)
                end
        ...
        if itemMsg.ItemType == 7 then
            ...
            if itemMsg.isDelivery == 1 then
                ...
                elseif itemMsg.ItemType == 3 then
                    ...
                    if itemMsg.ItemID == 9998 or (itemMsg.ItemID >=30000 and itemMsg.ItemID <40000)  then
                        ...
                        jc:registerScriptHandler( function()
                            ...
                            if array1.code>0 then -- 代表此用户信息存在 <0此用户信息不存在 msg代表具体原因
                                ...
                                --------------laixia.LocalPlayercfg.LaixiaCurrentWindow = "MyBagWindow"
                                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_REDEEMENTITY_WINDOW,data1)
                                if itemMsg.isDisapear == 1 then
                                    self.isPrevious = nil
                                    --清除右边数据
                                    self:GetWidgetByName("Mybag_Libao_Desc"):setString("")
                                ...
                                print(array1.msg)
                                -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"请关注公众号:" .. laixia.LocalPlayercfg.LaixiaWechatServiceNum)
                                -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                                --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,array1.msg)
                                ----------弹出 温馨提示界面 显示公众号label 隐藏请绑定手机号label 
                                ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_EXCHANGETISHI_WINDOW, 2)
                                 --  local data1 = {}
                                 -- data1.ID = sender.ItemID
                                 -- data1.ObjID = sender.ItemObjectID
                                 -- data1.page = "MyBagWindow"
                    ...
                    --                 end
                    --                 --     --TIPS
                    --                 -- elseif array2.code== -1 then  
                    --                 --     print("unionid为空")
                    --                 --     --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"unionid不能为空!")
                    --                 -- elseif array2.code == -2 then
                    --                 --     print("无此unionid")
                    --                 --     --ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"无此unionid!")
                    --                 -- elseif array2.code == -3 then
                    --                 --     print("红包金额错误，红包金额应在100~200元之间")
                    --                 --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"红包金额错误，红包金额应在100~200元之间!")
                    --                 --     --alert("Problem retrieving XML data");
                    --                 -- elseif array2.code == -5 then
                    --                 --     print("服务端配置错误")
                    --                 --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"服务端配置错误!")
                    --                 -- elseif array2.code == -6 then
                    --                 --     print("此用户未在微信公众号绑定")
                    --                 --   --  ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"此用户未在微信公众号绑定!")
                    --                 -- elseif array2.code == -7 then
                    --                 --     print("红包发放失败")
                    --                 --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"红包发放失败!")
                    --                 -- elseif array2.code == -4 then
                    --                 --     print("红包道具为空")
                    --                 --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"红包道具为空!")
                    --                 -- elseif array2.code == -8 then
                    --                 --     print("红包数量不足")
                    --                 --    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDE_RECONNECTIONTIPS_WINDOW)
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"红包数量不足!")
                    --                 -- elseif array2.code == -9 then
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"发放失败，请稍后重试!")
                    --                 -- elseif array2.code == -10 then
                    --                 --     ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_FLOWWORDS_WINDOW,"请绑定手机号后再使用红包道具!")
                    --                 -- end
                    --             end)
                    --             xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
                    --             local accessTokenURL = string.format("http://wx.laixia.com/posthb?unionid=%s&propid=%s&objectid=%s&phone=%s",unionid,pid,sender.ItemObjectID,laixia.LocalPlayercfg.LaixiaPhoneNum)
...
function MyBagWindow:onShow()
    -- ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDES_BAGTIPS_WINDOW) --隐藏背包tips
    ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_HIDES_BAGRED_WINDOW) --隐藏红点
...
--        if laixia.LocalPlayercfg.LaixiaPropsData and #laixia.LocalPlayercfg.LaixiaPropsData>0 then
--            self:hideTips()
--        else
--            self:showTips()
--        end
...
function MyBagWindow:updateWindow()
    if self.mIsShow then
        ...
        local itemMsg = laixia.JsonTxtData:queryTable("items")
...
function MyBagWindow:addItems()
    ...
    for listViewIndex=1,4 do-----现在只走listViewIndex==1（就是显示全部Item）
        ...
        local itemid = dataArrays[i].ItemID
            local itemMsg = laixia.JsonTxtData:queryTable("items"):query("ItemID",itemid); 
        ...
        for i=1,addModelNumber do 
            ...
            for j=0,4 do 
                ...
                if (dataArray[index] ~=nil and dataArray[index].ItemCount >= 0 )then
                    local itemMsg = laixia.JsonTxtData:queryTable("items"):query("ItemID",dataArray[index].ItemID);
...
function MyBagWindow:addItem(index, item,tabIndex)
    ...
    local itemData =laixia.JsonTxtData:queryTable("items"):query("ItemID",item.ItemID);
...
function MyBagWindow:onPresent(sender, eventType)
    if eventType == ccui.TouchEventType.ended then
        ...
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_PRESENT_WINDOW,data);
    end
end

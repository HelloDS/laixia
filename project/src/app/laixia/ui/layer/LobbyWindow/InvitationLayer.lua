...
function InvitationLayer:onInit()
    ...
    ObjectEventDispatch:addEventListener("_LAIXIA_EVENT_SHOW_INVITATION_WINDOW", handler(self, self.show))
    -- ObjectEventDispatch:addEventListener("_LAIXIA_EVENT_HIDE_INVITATION_WINDOW", handler(self, self.destroy))
end
...
--    end);
    ...
    if laixia.kconfig.isYingKe ~= true then
        ...
        self.TextField_invitation1:setLocalZOrder(100)
...
function InvitationLayer:Submit(sender, eventType)
    if eventType == ccui.TouchEventType.ended then 
        ...
        local str = "yu7#)"..parentPid..laixia.LocalPlayercfg.LaixiaPlayerID.."54=%~m"
        ...
        request:addPOSTValue("user_id2",laixia.LocalPlayercfg.LaixiaPlayerID)
...
function InvitationLayer:onResponseGet(event)
    ...
    if code ~= 200 then -- 成功  
        ...
        ObjectEventDispatch:pushEvent(_LAIXIA_EVENT_SHOW_MARKEDWORDS_WINDOW, array.MSG)   
    end  
end

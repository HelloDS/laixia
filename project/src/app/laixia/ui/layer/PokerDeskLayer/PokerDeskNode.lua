...
function PokerDeskNode:ctor( data )
    ...
    self:OnClick(self.Image_gang, function()
       ...
       -- pkModel.mPlayPoptyTb[sum]:GangPai( pkModel.mPlChupaiVal,1 )
        local layer = pkModel.mPlayPoptyTb[sum]
        xzmj.net.PokerHttp:SendOperation( layer.mOpcard ,pkcf.MJOPERATE.gang )
    ...
    self:OnClick(self.Image_peng, function()
        ...
        self:HidePGHGui()
        local layer = pkModel.mPlayPoptyTb[sum]
        xzmj.net.PokerHttp:SendOperation( layer.mOpcard ,pkcf.MJOPERATE.peng )
    end,{["isScale"] = false, ["hasAudio"] = false})
    ...
    self:OnClick(self.Image_guo, function()
        ...
        self:HidePGHGui()
        local layer = pkModel.mPlayPoptyTb[sum]
        xzmj.net.PokerHttp:SendOperation( layer.mOpcard ,pkcf.MJOPERATE.guo )
    end,{["isScale"] = false, ["hasAudio"] = false})    
